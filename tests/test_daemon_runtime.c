...
static void runtime_test_conflict_log_fallback_sink(const char *line) {
    if (line && strstr(line, "daemon.conflict_log_append_failed")) {
        atomic_store_explicit(&runtime_conflict_log_fallback_seen, true,
                              memory_order_release);
    }
}

static void runtime_test_activation_shutdown_sink(const char *line) {
    if (line && strstr(line, "daemon.activation_shutdown") &&
        strstr(line, "requester_pid") && strstr(line, "requester_build") &&
        strstr(line, "action") && strstr(line, "active_clients") &&
        strstr(line, "active_connections")) {
        atomic_store_explicit(&runtime_activation_shutdown_log_seen, true,
                              memory_order_release);
    }
}
...
static cbm_daemon_build_identity_t runtime_test_identity(const char *version,
                                                         const char *build) {
    cbm_daemon_build_identity_t identity = {
...
static bool runtime_test_windows_copy_self(const char *destination) {
    ...
    DWORD source_length = GetModuleFileNameW(
        NULL, source, (DWORD)(sizeof(source) / sizeof(source[0])));
...
static bool runtime_test_windows_posix_replace(const char *source,
                                               const char *destination) {
    wchar_t *source_wide = cbm_utf8_to_wide(source);
    wchar_t *destination_wide = cbm_utf8_to_wide(destination);
    size_t destination_length =
        destination_wide ? wcslen(destination_wide) : 0;
    size_t destination_bytes = destination_length * sizeof(wchar_t);
    size_t information_size =
        offsetof(runtime_test_windows_rename_info_t, file_name) +
        destination_bytes + sizeof(wchar_t);
    bool sizes_ok = destination_length > 0 &&
                    destination_bytes <= MAXDWORD &&
                    information_size <= MAXDWORD;
    runtime_test_windows_rename_info_t *information =
        sizes_ok ? calloc(1, information_size) : NULL;
    if (information) {
        information->flags =
        ...
        memcpy(information->file_name, destination_wide,
               destination_bytes + sizeof(wchar_t));
    }
    HANDLE source_file =
    ...
    bool replaced =
        ...
        SetFileInformationByHandle(
            source_file,
            (FILE_INFO_BY_HANDLE_CLASS)RUNTIME_TEST_FILE_RENAME_INFO_EX,
...
static bool runtime_test_windows_spawn_image_holder(
    const char *image_path, const char *ready_event,
    ...
    int written = snprintf(command_line, sizeof(command_line),
                           "\"%s\" __cbm_runtime_image_holder \"%s\"",
                           image_path, ready_event);
    wchar_t *application = cbm_utf8_to_wide(image_path);
    wchar_t *command = written > 0 && written < (int)sizeof(command_line)
                           ? cbm_utf8_to_wide(command_line)
                           : NULL;
    STARTUPINFOW startup;
    memset(&startup, 0, sizeof(startup));
    startup.cb = sizeof(startup);
    bool created = application && command &&
                   CreateProcessW(application, command, NULL, NULL, FALSE,
...
static bool runtime_test_copy_executable(const char *source,
                                         const char *destination) {
    int source_fd = open(source, O_RDONLY | O_CLOEXEC);
    int destination_fd = source_fd >= 0
                             ? open(destination,
    ...
    while (ok) {
        ...
        while (written < (size_t)count) {
            ssize_t chunk = write(destination_fd, buffer + written,
...
static pid_t runtime_test_spawn_blocked_executable(const char *path,
                                                   int *release_fd_out) {
    int ready[2] = {-1, -1};
    int input[2] = {-1, -1};
    ...
    (void)close(ready[0]);
    int status = 0;
    bool running = child > 0 && ready_count == 0 &&
...
#if defined(__APPLE__) || defined(__linux__)
static bool runtime_test_self_image_path(
...
#else
    ssize_t length =
        readlink("/proc/self/exe", source, RUNTIME_TEST_PATH_CAP - 1);
    bool resolved = length > 0 &&
                    length < (ssize_t)RUNTIME_TEST_PATH_CAP - 1;
    if (resolved) {
        source[length] = '\0';
    }
#endif
...
static bool runtime_test_mac_ad_hoc_sign(const char *path) {
    ...
    if (child == 0) {
        execl("/usr/bin/codesign", "codesign", "--force", "--sign", "-",
              "--timestamp=none", "--identifier",
              "org.deusdata.cbm.foreign-test", path, (char *)NULL);
        _exit(127);
    }
    int status = 0;
    pid_t waited;
...
static bool runtime_test_run_hello_image(
    const char *image_path, const runtime_test_fixture_t *fixture,
    const cbm_daemon_build_identity_t *identity, int *exit_code_out) {
    ...
    *exit_code_out = -1;
    ...
    int written = snprintf(
        ...
        "\"%s\" __cbm_runtime_hello_client \"%s\" %s %s %s", image_path,
    ...
    pid_t child = fork();
    if (child == 0) {
        execl(image_path, image_path, "__cbm_runtime_hello_client",
              fixture->parent, fixture->key, identity->semantic_version,
...
static bool runtime_test_run_activation_image(
    const char *image_path, const runtime_test_fixture_t *fixture,
    const cbm_daemon_build_identity_t *identity,
    cbm_daemon_runtime_activation_action_t action, int *exit_code_out) {
    if (!image_path || !fixture || !identity || !identity->semantic_version ||
        !identity->build_fingerprint || !exit_code_out) {
        return false;
    }
    *exit_code_out = -1;
    ...
    int written = snprintf(
        command_line, sizeof(command_line),
        "\"%s\" __cbm_runtime_activation_client \"%s\" %s %s %s %u",
        image_path, fixture->parent, fixture->key,
...
#elif defined(__APPLE__) || defined(__linux__)
    char action_text[16];
    int action_written = snprintf(action_text, sizeof(action_text), "%u",
                                  (unsigned int)action);
    ...
    if (child == 0) {
        (void)alarm(10);
        execl(image_path, image_path, "__cbm_runtime_activation_client",
              fixture->parent, fixture->key, identity->semantic_version,
              identity->build_fingerprint, action_text, (char *)NULL);
        _exit(127);
    }
    int status = 0;
    pid_t waited;
...
static bool runtime_test_run_mapped_hello_image(
    const char *image_path, const char *mapped_image_path,
    const runtime_test_fixture_t *fixture,
    const cbm_daemon_build_identity_t *identity, int *exit_code_out) {
    if (!image_path || !mapped_image_path || !fixture || !identity ||
        !identity->semantic_version || !identity->build_fingerprint ||
        !exit_code_out) {
        return false;
    }
    *exit_code_out = -1;
    pid_t child = fork();
    if (child == 0) {
        execl(image_path, image_path, "__cbm_runtime_mapped_hello_client",
              mapped_image_path, fixture->parent, fixture->key,
              identity->semantic_version, identity->build_fingerprint,
              (char *)NULL);
        _exit(127);
    }
...
static cbm_daemon_runtime_application_session_t *runtime_application_session_open(
    void *opaque, cbm_daemon_client_id_t client_id,
    uint64_t authenticated_process_id) {
    ...
    int open_index = atomic_fetch_add_explicit(&context->opened, 1,
                                               memory_order_relaxed);
    if (open_index == 1 &&
        atomic_load_explicit(&context->block_second_open,
                             memory_order_acquire)) {
        atomic_store_explicit(&context->second_open_started, true,
                              memory_order_release);
        while (!atomic_load_explicit(&context->release_second_open,
                                     memory_order_acquire)) {
...
static cbm_daemon_runtime_application_status_t runtime_application_request(
    ...
    (void)request_token;
    runtime_application_context_t *context = opaque;
    runtime_application_session_t *session =
        (runtime_application_session_t *)opaque_session;
    ...
    *response_length_out = 0;
    int request_index =
        atomic_fetch_add_explicit(&context->requests, 1, memory_order_relaxed);
    if (request_index == 0 &&
        atomic_load_explicit(&context->block_first_request,
                             memory_order_acquire)) {
        atomic_store_explicit(&context->first_request_started, true,
                              memory_order_release);
        bool ignore_cancel = atomic_load_explicit(
            &context->ignore_first_request_cancel, memory_order_acquire);
        while (!(ignore_cancel
                     ? atomic_load_explicit(&context->release_first_request,
                                            memory_order_acquire)
                     : atomic_load_explicit(&session->cancel_requested,
                                            memory_order_acquire))) {
            struct timespec pause = {.tv_sec = 0, .tv_nsec = 1000000};
            (void)cbm_nanosleep(&pause, NULL);
        }
...
static void *runtime_application_client_request_thread(void *opaque) {
    ...
    call->status = call->tagged
                       ? cbm_daemon_runtime_client_application_request_tagged(
                             call->client, call->request_token, call->request,
                             call->request_length, &call->response,
...
static void *runtime_application_client_connect_thread(void *opaque) {
    ...
    call->client = cbm_daemon_runtime_client_connect(
        call->endpoint, &call->identity, RUNTIME_TEST_TIMEOUT_MS,
        &call->result);
    atomic_store_explicit(&call->completed, true, memory_order_release);
    return NULL;
}
...
static void runtime_application_request_cancel(
    ...
    atomic_store_explicit(&session->cancel_requested, true,
                          memory_order_release);
    (void)atomic_fetch_add_explicit(&context->request_cancels, 1,
                                    memory_order_relaxed);
}
...
static void runtime_application_session_cancel(
    ...
    atomic_store_explicit(&session->cancel_requested, true,
                          memory_order_release);
    (void)atomic_fetch_add_explicit(&context->cancelled, 1,
                                    memory_order_relaxed);
}
...
static void runtime_application_session_close(
    ...
    (void)atomic_fetch_add_explicit(&context->closed, 1,
                                    memory_order_relaxed);
    free(session);
}
...
static bool runtime_test_wait_atomic_bool(atomic_bool *value,
                                         uint32_t timeout_ms) {
...
static bool runtime_test_wait_atomic_int(atomic_int *value, int expected,
                                         uint32_t timeout_ms) {
    uint64_t deadline = cbm_now_ms() + (uint64_t)timeout_ms;
...
static bool runtime_test_raw_activation_exchange(
    const cbm_daemon_ipc_endpoint_t *endpoint, const uint8_t *request,
    uint32_t request_length,
    ...
    bool sent = connection && cbm_daemon_ipc_send_frame(
                                   connection, CBM_DAEMON_FRAME_REQUEST,
                                   CBM_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN,
                                   request, request_length);
    cbm_daemon_frame_t frame = {0};
    uint8_t *response = NULL;
    int received = sent ? cbm_daemon_ipc_receive_frame(
                              connection, RUNTIME_TEST_TIMEOUT_MS, &frame,
                              &response)
                        : 0;
    uint32_t status = received == 1 && response
                          ? runtime_test_get_u32(response + 4)
                          : UINT32_MAX;
    bool valid = received == 1 &&
                 frame.type == CBM_DAEMON_FRAME_RESPONSE &&
                 frame.flags == CBM_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN &&
                 frame.length == RUNTIME_TEST_ACTIVATION_RESPONSE_SIZE &&
                 response &&
                 runtime_test_get_u32(response) ==
                     RUNTIME_TEST_RENDEZVOUS_ABI &&
                 status <= 1U;
    if (valid) {
        result_out->accepted = status == 1U;
        result_out->active_clients = runtime_test_get_u64(
            response + RUNTIME_TEST_ACTIVATION_RESPONSE_CLIENTS_OFFSET);
        result_out->active_connections = runtime_test_get_u64(
...
static bool runtime_test_activation_request_encode(
    ...
    return cbm_daemon_runtime_hello_request_encode(
        out + RUNTIME_TEST_ACTIVATION_IDENTITY_OFFSET, identity);
}

static bool runtime_test_fixed_string_equals(const uint8_t *wire,
                                             size_t capacity,
                                             const char *expected) {
    if (!wire || !expected) {
        return false;
    ...
    if (length >= capacity || memcmp(wire, expected, length) != 0 ||
        wire[length] != 0) {
        return false;
    }
...
static cbm_daemon_ipc_connection_t *runtime_test_raw_client_connect(
    ...
    int received = cbm_daemon_ipc_receive_frame(connection,
                                                RUNTIME_TEST_TIMEOUT_MS,
                                                &frame, &payload);
...
static bool runtime_test_raw_application_send_token(
    cbm_daemon_ipc_connection_t *connection, uint64_t request_token,
    const void *payload, uint32_t payload_length, uint32_t declared_length) {
    ...
    bool sent = cbm_daemon_ipc_send_frame(
        connection, CBM_DAEMON_FRAME_REQUEST,
...
static bool runtime_test_raw_application_send(
    cbm_daemon_ipc_connection_t *connection, const void *payload,
    uint32_t payload_length, uint32_t declared_length) {
    static atomic_uint_fast64_t next_token = ATOMIC_VAR_INIT(1);
    uint64_t request_token = atomic_fetch_add_explicit(
        &next_token, 1, memory_order_relaxed);
    return runtime_test_raw_application_send_token(
        connection, request_token, payload, payload_length, declared_length);
}

static bool runtime_test_raw_application_cancel(
    cbm_daemon_ipc_connection_t *connection, uint64_t request_token) {
    uint8_t wire[8];
    ...
    runtime_test_put_u64(wire, request_token);
    return cbm_daemon_ipc_send_frame(
        connection, CBM_DAEMON_FRAME_REQUEST,
...
static bool runtime_test_raw_application_receive_status_token(
    ...
    bool valid = received == 1 && frame.type == CBM_DAEMON_FRAME_RESPONSE &&
                 ...
                 frame.length >= 16 && payload &&
                 runtime_test_get_u64(payload) == expected_token &&
                 runtime_test_get_u32(payload + 8) ==
...
static bool runtime_test_raw_application_receive_status(
    ...
    bool valid = received == 1 && frame.type == CBM_DAEMON_FRAME_RESPONSE &&
                 frame.flags == CBM_DAEMON_RUNTIME_OP_APPLICATION_REQUEST &&
                 frame.length >= 16 && payload &&
                 runtime_test_get_u64(payload) != 0 &&
                 runtime_test_get_u32(payload + 8) ==
                     (uint32_t)expected_status &&
                 runtime_test_get_u32(payload + 12) == frame.length - 16;
    free(payload);
...
static bool runtime_test_fixture_start_failed(const char *tag,
                                              const char *stage,
                                              long long detail) {
    printf("  runtime fixture startup failed: tag=%s stage=%s detail=%lld\n",
           tag ? tag : "(null)", stage ? stage : "(null)", detail);
    return false;
}

static bool runtime_test_fixture_start_configured(
    runtime_test_fixture_t *fixture, const char *tag,
    const cbm_daemon_build_identity_t *identity, uint32_t max_clients,
    ...
    if (parent_written <= 0 || parent_written >= (int)sizeof(fixture->parent)) {
        return runtime_test_fixture_start_failed(tag, "temporary-path",
                                                 parent_written);
    }
    if (!cbm_mkdtemp(fixture->parent)) {
        return runtime_test_fixture_start_failed(
            tag, "temporary-directory", runtime_test_last_os_error());
    }

    if (!cbm_daemon_rendezvous_key(fixture->key)) {
        return runtime_test_fixture_start_failed(
            tag, "rendezvous-key", runtime_test_last_os_error());
    }
    fixture->endpoint =
        cbm_daemon_ipc_endpoint_new(fixture->key, fixture->parent);
    if (!fixture->endpoint) {
        return runtime_test_fixture_start_failed(
            tag, "endpoint", runtime_test_last_os_error());
    }
    if (!runtime_test_copy_path(
            fixture->runtime_dir,
    ...
    int log_written = snprintf(fixture->log_path, sizeof(fixture->log_path), "%s/conflicts.ndjson",
                               fixture->parent);
    int rotated_written = snprintf(fixture->rotated_log_path,
                                   sizeof(fixture->rotated_log_path), "%s.1",
                                   fixture->log_path);
    int lock_written = snprintf(fixture->lock_log_path,
                                sizeof(fixture->lock_log_path), "%s.lock",
                                fixture->log_path);
    if (log_written <= 0 || log_written >= (int)sizeof(fixture->log_path)) {
        return runtime_test_fixture_start_failed(tag, "conflict-log-path",
                                                 log_written);
    }
    if (rotated_written <= 0 ||
        rotated_written >= (int)sizeof(fixture->rotated_log_path)) {
        return runtime_test_fixture_start_failed(tag, "rotated-log-path",
                                                 rotated_written);
    }
    if (lock_written <= 0 ||
        lock_written >= (int)sizeof(fixture->lock_log_path)) {
        return runtime_test_fixture_start_failed(tag, "lock-log-path",
                                                 lock_written);
    }
    ...
    if (!fixture->service) {
        return runtime_test_fixture_start_failed(
            tag, "service-start", runtime_test_last_os_error());
    }
    ...
    if (state != CBM_DAEMON_RUNTIME_SERVICE_RUNNING) {
        return runtime_test_fixture_start_failed(tag, "service-state",
                                                 (long long)state);
    }
    return true;
}

static bool runtime_test_fixture_start_limited(runtime_test_fixture_t *fixture,
                                               const char *tag,
                                               const cbm_daemon_build_identity_t *identity,
                                               uint32_t max_clients) {
    return runtime_test_fixture_start_configured(fixture, tag, identity,
                                                 max_clients, 5000, NULL);
}
...
TEST(daemon_runtime_convenience_service_owns_participant_guard) {
    ...
    int active = started
                     ? cbm_daemon_ipc_legacy_generation_probe(fixture.endpoint)
                     : -1;
    bool running_free_refused =
        started && !cbm_daemon_runtime_service_free(fixture.service);
    bool stopped = started && cbm_daemon_runtime_service_stop(
                                  fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    bool freed = stopped &&
                 cbm_daemon_runtime_service_free(fixture.service);
    if (freed) {
...
static bool runtime_test_fixture_start_application(
    runtime_test_fixture_t *fixture, const char *tag,
    const cbm_daemon_build_identity_t *identity,
    runtime_application_context_t *context) {
    cbm_daemon_runtime_application_callbacks_t application =
        runtime_application_callbacks(context);
    return runtime_test_fixture_start_configured(fixture, tag, identity, 8,
                                                 5000, &application);
}
...
static void runtime_test_fixture_finish(runtime_test_fixture_t *fixture) {
    if (fixture->service) {
        ...
        if (state != CBM_DAEMON_RUNTIME_SERVICE_EXITED) {
            (void)cbm_daemon_runtime_service_stop(fixture->service,
                                                  RUNTIME_TEST_TIMEOUT_MS);
        }
...
#ifndef _WIN32
static int runtime_test_failed_host_child(const char *parent,
                                          const char *key) {
    ...
    int log_written = snprintf(log_path, sizeof(log_path),
                               "%s/logs/cbm-daemon.log", cache);
    if (cache_written <= 0 || cache_written >= (int)sizeof(cache) ||
    ...
    cbm_daemon_build_identity_t mismatched = runtime_test_identity(
        "host-order-test", RUNTIME_BUILD_B);
    ...
    bool failed_at_runtime =
        read && strstr(log, "daemon.start_failed") != NULL &&
        strstr(log, "runtime") != NULL;
...
TEST(daemon_host_failed_listener_reservation_starts_no_background_work) {
    char parent[RUNTIME_TEST_PATH_CAP];
    int written = snprintf(parent, sizeof(parent),
                           "%s/cbm-host-order-XXXXXX", cbm_tmpdir());
    bool parent_created = written > 0 && written < (int)sizeof(parent) &&
                          cbm_mkdtemp(parent) != NULL;
    char key[CBM_DAEMON_KEY_SIZE] = {0};
    cbm_daemon_ipc_endpoint_t *endpoint =
        parent_created && cbm_daemon_rendezvous_key(key)
            ? cbm_daemon_ipc_endpoint_new(key, parent)
            : NULL;
    cbm_daemon_ipc_startup_lock_t *startup = NULL;
    bool setup = endpoint &&
                 cbm_daemon_ipc_startup_lock_try_acquire(endpoint, &startup) == 1 &&
...
static _Noreturn void runtime_test_forced_host_shutdown_child(
    const char *parent) {
    char cache[RUNTIME_TEST_PATH_CAP];
...
static _Noreturn void runtime_test_persistent_cleanup_failure_child(
    const char *parent) {
    char cache[RUNTIME_TEST_PATH_CAP];
    int written = snprintf(cache, sizeof(cache), "%s/cache", parent);
    if (written <= 0 || written >= (int)sizeof(cache) ||
...
TEST(daemon_host_persistent_cleanup_release_failure_is_process_bounded) {
    char parent[RUNTIME_TEST_PATH_CAP];
    char log_path[RUNTIME_TEST_PATH_CAP];
    int parent_written = snprintf(parent, sizeof(parent),
                                  "%s/cbm-host-cleanup-XXXXXX", cbm_tmpdir());
    bool parent_created = parent_written > 0 &&
                          parent_written < (int)sizeof(parent) &&
                          cbm_mkdtemp(parent) != NULL;
...
TEST(daemon_host_forced_shutdown_is_logged_flushed_and_process_bounded) {
    ...
    char log_path[RUNTIME_TEST_PATH_CAP];
    int parent_written = snprintf(parent, sizeof(parent),
                                  "%s/cbm-host-force-XXXXXX", cbm_tmpdir());
    bool parent_created = parent_written > 0 &&
                          parent_written < (int)sizeof(parent) &&
                          cbm_mkdtemp(parent) != NULL;
    int log_written = parent_created
...
TEST(daemon_runtime_exact_hello_issues_connection_bound_identity) {
    ...
    if (client) {
        ...
        identity_anchored =
            result.authenticated_process_id == expected_pid &&
            cbm_daemon_runtime_client_process_id(client) == expected_pid &&
            cbm_daemon_runtime_service_client_process_id(fixture.service, result.client_id) ==
                expected_pid;
        closed = cbm_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS);
        client = NULL;
        exited = cbm_daemon_runtime_service_wait_exited(fixture.service,
                                                        RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_activation_rejects_forged_and_malformed_without_stop) {
    cbm_daemon_build_identity_t identity =
        runtime_test_identity("2.4.0", runtime_test_self_build());
    cbm_daemon_build_identity_t forged =
        runtime_test_identity("9.9.9", RUNTIME_BUILD_B);
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start(
        &fixture, "activation-reject", &identity);
    ...
    if (started) {
        owner = cbm_daemon_runtime_client_connect(
            fixture.endpoint, &identity, RUNTIME_TEST_TIMEOUT_MS,
            &owner_result);
    }
    if (owner && encoded) {
        short_rejected = runtime_test_raw_activation_exchange(
                             fixture.endpoint, request,
                             RUNTIME_TEST_ACTIVATION_REQUEST_SIZE - 1U,
                             &short_result) &&
                         !short_result.accepted;
        runtime_test_put_u32(request, UINT32_C(99));
        action_rejected = runtime_test_raw_activation_exchange(
                              fixture.endpoint, request,
                              RUNTIME_TEST_ACTIVATION_REQUEST_SIZE,
                              &action_result) &&
                          !action_result.accepted;
        runtime_test_put_u32(
            request, (uint32_t)CBM_DAEMON_RUNTIME_ACTIVATION_UPDATE);
        runtime_test_put_u32(
            request + RUNTIME_TEST_ACTIVATION_IDENTITY_OFFSET,
        ...
        unchanged =
            ...
            cbm_daemon_runtime_service_wait_for_connections(
                fixture.service, 1, RUNTIME_TEST_TIMEOUT_MS);
        heartbeat = cbm_daemon_runtime_client_heartbeat(
            owner, RUNTIME_TEST_TIMEOUT_MS);
    }
    if (owner) {
        closed = cbm_daemon_runtime_client_close(
            owner, RUNTIME_TEST_TIMEOUT_MS);
        owner = NULL;
    }
    if (started) {
        exited = cbm_daemon_runtime_service_wait_exited(
            fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
    runtime_test_fixture_finish(&fixture);
...
TEST(daemon_runtime_activation_ack_snapshots_then_interrupts_all_clients) {
    ...
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start(
        &fixture, "activation-drain", &identity);
    ...
    bool exited = false;
    atomic_store_explicit(&runtime_activation_shutdown_log_seen, false,
                          memory_order_release);
    cbm_log_set_sink(runtime_test_activation_shutdown_sink);

    if (started) {
        first = cbm_daemon_runtime_client_connect(
            fixture.endpoint, &identity, RUNTIME_TEST_TIMEOUT_MS,
            &first_result);
        second = cbm_daemon_runtime_client_connect(
            fixture.endpoint, &identity, RUNTIME_TEST_TIMEOUT_MS,
            &second_result);
    }
    if (first && second) {
        ...
        first_interrupted = !cbm_daemon_runtime_client_heartbeat(
            first, RUNTIME_TEST_TIMEOUT_MS);
        second_interrupted = !cbm_daemon_runtime_client_heartbeat(
            second, RUNTIME_TEST_TIMEOUT_MS);
    }
    cbm_log_set_sink(NULL);
    if (first) {
        (void)cbm_daemon_runtime_client_close(first,
                                              RUNTIME_TEST_TIMEOUT_MS);
        first = NULL;
    }
    if (second) {
        (void)cbm_daemon_runtime_client_close(second,
                                              RUNTIME_TEST_TIMEOUT_MS);
        second = NULL;
    }
    if (started) {
    ...
    ASSERT_TRUE(atomic_load_explicit(&runtime_activation_shutdown_log_seen,
                                     memory_order_acquire));
...
TEST(daemon_runtime_activation_accepts_authenticated_different_build) {
    ...
    int directory_written = snprintf(
        directory, sizeof(directory), "%s/cbm-runtime-activation-XXXXXX",
        cbm_tmpdir());
    bool directory_created = directory_written > 0 &&
                             directory_written < (int)sizeof(directory) &&
                             cbm_mkdtemp(directory) != NULL;
    int image_written = directory_created
                            ? snprintf(foreign_image, sizeof(foreign_image),
                                       "%s/foreign-activation", directory)
                            : -1;
    bool copied = image_written > 0 &&
                  image_written < (int)sizeof(foreign_image) &&
    ...
    bool fingerprinted =
        runnable && cbm_daemon_build_fingerprint_file(foreign_image,
                                                      foreign_build);
    bool differs = fingerprinted &&
                   strcmp(foreign_build,
                          active_identity.build_fingerprint) != 0;
    cbm_daemon_build_identity_t foreign_identity =
        runtime_test_identity("9.9.9", foreign_build);
    ...
    runtime_test_fixture_t fixture;
    memset(&fixture, 0, sizeof(fixture));
    bool started = differs && runtime_test_fixture_start(
                                  &fixture, "activation-foreign",
                                  &active_identity);
    cbm_daemon_runtime_connect_result_t owner_result = {0};
    cbm_daemon_runtime_client_t *owner = NULL;
    if (started) {
        owner = cbm_daemon_runtime_client_connect(
            fixture.endpoint, &active_identity, RUNTIME_TEST_TIMEOUT_MS,
            &owner_result);
    }
    int foreign_exit = -1;
    bool foreign_ran = owner && differs &&
                       runtime_test_run_activation_image(
                           foreign_image, &fixture, &foreign_identity,
    ...
    bool owner_interrupted = owner && foreign_ran &&
                             !cbm_daemon_runtime_client_heartbeat(
                                 owner, RUNTIME_TEST_TIMEOUT_MS);
    if (owner) {
        (void)cbm_daemon_runtime_client_close(owner,
                                              RUNTIME_TEST_TIMEOUT_MS);
        owner = NULL;
    }
    bool exited = started && cbm_daemon_runtime_service_wait_exited(
                               fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    runtime_test_fixture_finish(&fixture);
...
TEST(daemon_runtime_rendezvous_layout_is_frozen_and_detailed_abi_independent) {
    cbm_daemon_build_identity_t first =
        runtime_test_identity("2.4.0", runtime_test_self_build());
    ...
    bool first_encoded =
        cbm_daemon_runtime_hello_request_encode(first_wire, &first);
    bool second_encoded = cbm_daemon_runtime_hello_request_encode(
        second_wire, &different_detail);

    ASSERT_TRUE(first_encoded);
    ASSERT_TRUE(second_encoded);
    ASSERT_EQ(CBM_DAEMON_RENDEZVOUS_REQUEST_SIZE,
              RUNTIME_TEST_RENDEZVOUS_REQUEST_SIZE);
    ASSERT_EQ(CBM_DAEMON_RENDEZVOUS_RESPONSE_SIZE,
              RUNTIME_TEST_RENDEZVOUS_RESPONSE_SIZE);
    ASSERT_EQ(CBM_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
              CBM_DAEMON_VERSION_TEXT_SIZE);
    ASSERT_EQ(CBM_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP,
              CBM_DAEMON_BUILD_FINGERPRINT_SIZE);
    ASSERT_EQ(CBM_DAEMON_RENDEZVOUS_MESSAGE_CAP,
              CBM_DAEMON_CONFLICT_MESSAGE_SIZE);
    ASSERT_EQ(runtime_test_get_u32(first_wire),
              RUNTIME_TEST_RENDEZVOUS_ABI);
    ASSERT_TRUE(runtime_test_fixed_string_equals(
        first_wire + RUNTIME_TEST_RENDEZVOUS_VERSION_OFFSET,
        CBM_DAEMON_VERSION_TEXT_SIZE, first.semantic_version));
    ASSERT_TRUE(runtime_test_fixed_string_equals(
        first_wire + RUNTIME_TEST_RENDEZVOUS_BUILD_OFFSET,
        CBM_DAEMON_BUILD_FINGERPRINT_SIZE, first.build_fingerprint));
...
TEST(daemon_runtime_future_generation_gets_stable_explicit_conflict) {
    ...
    cbm_daemon_build_identity_t active =
        runtime_test_identity("2.4.0", active_build);
    ...
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start(
        &fixture, "future-generation", &active);
    cbm_daemon_runtime_connect_result_t owner_result = {0};
    cbm_daemon_runtime_client_t *owner = NULL;
    ...
    int expected_length = snprintf(
        ...
        active.semantic_version, active_build, future.semantic_version,
        future.build_fingerprint);
    bool expected_message_valid =
        expected_length > 0 && (size_t)expected_length < sizeof(expected_message);

    if (started) {
        owner = cbm_daemon_runtime_client_connect(
            fixture.endpoint, &active, RUNTIME_TEST_TIMEOUT_MS, &owner_result);
    }
    if (owner) {
        encoded = cbm_daemon_runtime_hello_request_encode(future_wire, &future);
        future_connection = cbm_daemon_ipc_connect(
            fixture.endpoint, RUNTIME_TEST_TIMEOUT_MS);
    }
    if (future_connection && encoded) {
        ...
        sent = cbm_daemon_ipc_send_frame(
            future_connection, CBM_DAEMON_FRAME_REQUEST,
            CBM_DAEMON_RUNTIME_OP_HELLO, future_wire,
            RUNTIME_TEST_RENDEZVOUS_REQUEST_SIZE);
        int received = cbm_daemon_ipc_receive_frame(
            future_connection, RUNTIME_TEST_TIMEOUT_MS, &response_frame,
            &response_payload);
        explicit_conflict =
            ...
            runtime_test_get_u32(response_payload) ==
                CBM_DAEMON_RUNTIME_CONNECT_CONFLICT &&
            runtime_test_get_u32(response_payload + 4) ==
            ...
            runtime_test_fixed_string_equals(
                response_payload + RUNTIME_TEST_RENDEZVOUS_ACTIVE_BUILD_OFFSET,
            ...
            runtime_test_fixed_string_equals(
                ...
                future.build_fingerprint) &&
            runtime_test_fixed_string_equals(
                response_payload + RUNTIME_TEST_RENDEZVOUS_MESSAGE_OFFSET,
...
TEST(daemon_runtime_matching_clients_share_one_service_endpoint) {
    ...
    if (same_endpoint) {
        same_address =
            strcmp(cbm_daemon_ipc_endpoint_address(fixture.endpoint),
    ...
    if (first && second) {
        ...
        first = NULL;
        one_remains = cbm_daemon_runtime_service_wait_for_clients(
            fixture.service, 1, RUNTIME_TEST_TIMEOUT_MS);
...
TEST(daemon_runtime_conflict_log_failure_uses_operation_log_fallback) {
    ...
    atomic_store_explicit(&runtime_conflict_log_fallback_seen, false,
                          memory_order_release);
    ...
    if (owner) {
        rejected = cbm_daemon_runtime_client_connect(
            fixture.endpoint, &rebuilt, RUNTIME_TEST_TIMEOUT_MS,
            &conflict_result);
        explicit_conflict =
            rejected == NULL &&
            conflict_result.status == CBM_DAEMON_RUNTIME_CONNECT_CONFLICT &&
    ...
    if (rejected) {
        (void)cbm_daemon_runtime_client_close(rejected,
                                              RUNTIME_TEST_TIMEOUT_MS);
    }
    if (owner) {
        (void)cbm_daemon_runtime_client_close(owner,
    ...
    bool fallback_seen = atomic_load_explicit(
        &runtime_conflict_log_fallback_seen, memory_order_acquire);
    if (obstruction_created) {
...
TEST(daemon_runtime_disconnect_releases_only_connection_subscriptions) {
    ...
    if (first && second) {
        cbm_daemon_subscription_result_t first_status =
            cbm_daemon_runtime_client_job_subscribe(first, project, &first_subscription,
                                                    RUNTIME_TEST_TIMEOUT_MS);
        cbm_daemon_subscription_result_t second_status =
            cbm_daemon_runtime_client_job_subscribe(second, project, &second_subscription,
                                                    RUNTIME_TEST_TIMEOUT_MS);
        ...
        (void)cbm_daemon_runtime_client_close(first, RUNTIME_TEST_TIMEOUT_MS);
        first = NULL;
        second_survived = cbm_daemon_runtime_service_wait_for_clients(
                              fixture.service, 1, RUNTIME_TEST_TIMEOUT_MS) &&
                          cbm_daemon_runtime_service_job_subscribers(fixture.service, project) ==
                              1 &&
                          cbm_daemon_runtime_client_job_unsubscribe(
                              second, second_subscription, RUNTIME_TEST_TIMEOUT_MS) &&
                          cbm_daemon_runtime_service_job_subscribers(fixture.service, project) ==
                              0;
...
TEST(daemon_runtime_authenticated_idle_connection_outlives_lease_interval) {
    cbm_daemon_build_identity_t identity =
        runtime_test_identity("2.4.0", runtime_test_self_build());
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start_configured(
        &fixture, "idle-connection", &identity, 8, 20, NULL);
    cbm_daemon_runtime_connect_result_t result = {0};
    cbm_daemon_runtime_client_t *client = NULL;
    ...
    if (client) {
        ...
        remained_connected =
            cbm_daemon_runtime_service_active_clients(fixture.service) == 1 &&
            cbm_daemon_runtime_client_heartbeat(client,
                                                RUNTIME_TEST_TIMEOUT_MS);
        (void)cbm_daemon_runtime_client_close(client,
                                              RUNTIME_TEST_TIMEOUT_MS);
        client = NULL;
        exited = cbm_daemon_runtime_service_wait_exited(
            fixture.service, RUNTIME_TEST_TIMEOUT_MS);
...
TEST(daemon_runtime_connection_cap_covers_slow_hello_and_stopping_is_terminal) {
    ...
    if (accepted) {
        ...
        capacity_rejected =
            overflow == NULL && overflow_result.status == CBM_DAEMON_RUNTIME_CONNECT_REJECTED &&
...
TEST(daemon_runtime_rejects_forged_identity_extension) {
    ...
    uint8_t forged[CBM_DAEMON_RENDEZVOUS_REQUEST_SIZE +
                   sizeof(uint64_t) * 2] = {0};
    ...
    if (started) {
        ...
        memcpy(forged + CBM_DAEMON_RENDEZVOUS_REQUEST_SIZE,
               &forged_client_id,
    ...
    if (raw && encoded) {
        sent = cbm_daemon_ipc_send_frame(raw, CBM_DAEMON_FRAME_REQUEST,
                                         CBM_DAEMON_RUNTIME_OP_HELLO, forged,
                                         (uint32_t)sizeof(forged));
        int received = cbm_daemon_ipc_receive_frame(raw, RUNTIME_TEST_TIMEOUT_MS,
...
TEST(daemon_runtime_application_response_roundtrip_is_byte_exact) {
    ...
    bool started = runtime_test_fixture_start_application(
        &fixture, "application-roundtrip", &identity, &context);
    cbm_daemon_runtime_connect_result_t result = {0};
    cbm_daemon_runtime_client_t *client = NULL;
    uint8_t *response = NULL;
    ...
    if (client) {
        status = cbm_daemon_runtime_client_application_request(
            client, request, (uint32_t)sizeof(request), &response,
            &response_length, RUNTIME_TEST_TIMEOUT_MS);
        exact = status == CBM_DAEMON_RUNTIME_APPLICATION_OK &&
                response_length == sizeof(request) && response &&
                memcmp(response, request, sizeof(request)) == 0;
        free(response);
        response = NULL;
        closed = cbm_daemon_runtime_client_close(client,
                                                 RUNTIME_TEST_TIMEOUT_MS);
        client = NULL;
        exited = cbm_daemon_runtime_service_wait_exited(
            fixture.service, RUNTIME_TEST_TIMEOUT_MS);
...
TEST(daemon_runtime_final_disconnect_rejects_blocked_provisional_session) {
    ...
    runtime_application_context_t context;
    runtime_application_context_init(&context, false);
    atomic_store_explicit(&context.block_second_open, true,
                          memory_order_release);
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start_application(
        &fixture, "application-open-race", &identity, &context);
    cbm_daemon_runtime_connect_result_t owner_result = {0};
    cbm_daemon_runtime_client_t *owner = NULL;
    ...
    if (owner) {
        ...
        connect_thread_create_rc = cbm_thread_create(
            &connect_thread, 128U * 1024U,
            runtime_application_client_connect_thread, &contender);
        connect_thread_started = connect_thread_create_rc == 0;
        provisional_started = connect_thread_started &&
                              runtime_test_wait_atomic_bool(
                                  &context.second_open_started,
                                  RUNTIME_TEST_TIMEOUT_MS);
    }
    if (provisional_started) {
        owner_closed = cbm_daemon_runtime_client_close(
            owner, RUNTIME_TEST_TIMEOUT_MS);
        owner = NULL;
    ...
    atomic_store_explicit(&context.release_second_open, true,
                          memory_order_release);
    ...
    if (connect_thread_started) {
        while (!atomic_load_explicit(&contender.completed,
                                     memory_order_acquire)) {
            struct timespec pause = {.tv_sec = 0, .tv_nsec = 1000000};
            (void)cbm_nanosleep(&pause, NULL);
        }
    ...
    if (contender.client) {
        (void)cbm_daemon_runtime_client_close(contender.client,
                                              RUNTIME_TEST_TIMEOUT_MS);
    ...
    ASSERT_TRUE(atomic_load_explicit(&contender.completed,
                                     memory_order_acquire));
...
TEST(daemon_runtime_request_cancel_is_exact_and_session_remains_usable) {
    ...
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start_application(
        &fixture, "application-request-cancel", &identity, &context);
    cbm_daemon_runtime_connect_result_t result = {0};
    cbm_daemon_runtime_client_t *client = NULL;
    cbm_daemon_runtime_application_token_t request_token =
    ...
    cbm_daemon_runtime_cancel_result_t wrong_cancel =
        CBM_DAEMON_RUNTIME_CANCEL_ERROR;
    int cancels_after_wrong = -1;
    cbm_daemon_runtime_cancel_result_t exact_cancel =
        CBM_DAEMON_RUNTIME_CANCEL_ERROR;
    bool cancel_delivered = false;
    cbm_daemon_runtime_cancel_result_t late_duplicate_cancel =
        CBM_DAEMON_RUNTIME_CANCEL_ERROR;
    ...
    if (started) {
        client = cbm_daemon_runtime_client_connect(
            fixture.endpoint, &identity, RUNTIME_TEST_TIMEOUT_MS, &result);
    }
    if (client) {
        token_reserved = cbm_daemon_runtime_client_application_token_reserve(
    ...
    if (token_reserved) {
        ...
        request_thread_create_rc = cbm_thread_create(
            &request_thread, 128U * 1024U,
            runtime_application_client_request_thread, &call);
        ...
        callback_started = request_thread_started &&
                           runtime_test_wait_atomic_bool(
                               &context.first_request_started,
                               RUNTIME_TEST_TIMEOUT_MS);
    }
    if (callback_started) {
        wrong_cancel = cbm_daemon_runtime_client_application_cancel(
            client, request_token + 1U);
        cancels_after_wrong = atomic_load_explicit(
            &context.request_cancels, memory_order_acquire);
        exact_cancel = cbm_daemon_runtime_client_application_cancel(
            client, request_token);
        cancel_delivered =
            exact_cancel == CBM_DAEMON_RUNTIME_CANCEL_ACCEPTED &&
            runtime_test_wait_atomic_int(&context.request_cancels, 1,
                                         RUNTIME_TEST_TIMEOUT_MS);
    }
    ...
    if (request_thread_joined && cancel_delivered && !close_begun) {
        late_duplicate_cancel =
            cbm_daemon_runtime_client_application_cancel(client,
                                                         request_token);
        cancels_after_duplicate = atomic_load_explicit(
            &context.request_cancels, memory_order_acquire);
        next_status = cbm_daemon_runtime_client_application_request(
            client, next_request, (uint32_t)sizeof(next_request),
            &next_response, &next_response_length,
            RUNTIME_TEST_TIMEOUT_MS);
        next_exact = next_status == CBM_DAEMON_RUNTIME_APPLICATION_OK &&
                     next_response_length == sizeof(next_request) &&
                     next_response &&
                     memcmp(next_response, next_request,
                            sizeof(next_request)) == 0;
        heartbeat = cbm_daemon_runtime_client_heartbeat(
    ...
    if (request_thread_started) {
        ...
        while (!atomic_load_explicit(&request_thread_completed,
                                     memory_order_acquire)) {
            struct timespec pause = {.tv_sec = 0, .tv_nsec = 1000000};
            (void)cbm_nanosleep(&pause, NULL);
        }
    ...
    if (client) {
        closed = close_begun
                     ? cbm_daemon_runtime_client_close_finish(
                           client, RUNTIME_TEST_TIMEOUT_MS)
                     : cbm_daemon_runtime_client_close(
                           client, RUNTIME_TEST_TIMEOUT_MS);
        client = NULL;
    }
    if (started) {
...
TEST(daemon_runtime_presend_request_cancel_is_sticky_and_nonterminal) {
    ...
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start_application(
        &fixture, "application-presend-cancel", &identity, &context);
    cbm_daemon_runtime_connect_result_t result = {0};
    cbm_daemon_runtime_client_t *client = NULL;
    cbm_daemon_runtime_application_token_t request_token =
        CBM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    bool token_reserved = false;
    cbm_daemon_runtime_cancel_result_t cancel =
        CBM_DAEMON_RUNTIME_CANCEL_ERROR;
    ...
    if (token_reserved) {
        cancel = cbm_daemon_runtime_client_application_cancel(
            client, request_token);
        cancelled_status =
            cbm_daemon_runtime_client_application_request_tagged(
                client, request_token, blocking_request,
                (uint32_t)sizeof(blocking_request), &cancelled_response,
                &cancelled_response_length, RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_allows_only_one_unstarted_application_token) {
    ...
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start_application(
        &fixture, "application-token-reservation", &identity, &context);
    cbm_daemon_runtime_connect_result_t result = {0};
    cbm_daemon_runtime_client_t *client = NULL;
    ...
    if (client) {
        first_reserved = cbm_daemon_runtime_client_application_token_reserve(
            client, &first_token);
        duplicate_reservation_rejected =
            !cbm_daemon_runtime_client_application_token_reserve(
                client, &rejected_token) &&
    ...
    if (first_reserved && duplicate_reservation_rejected) {
        ...
        cbm_daemon_runtime_application_status_t status =
            cbm_daemon_runtime_client_application_request_tagged(
                client, first_token, first_request,
                (uint32_t)sizeof(first_request), &response, &response_length,
                RUNTIME_TEST_TIMEOUT_MS);
        first_exact = status == CBM_DAEMON_RUNTIME_APPLICATION_OK &&
                      response_length == sizeof(first_request) && response &&
                      memcmp(response, first_request, sizeof(first_request)) ==
                          0;
        free(response);
    }
    if (first_exact) {
    ...
    if (second_reserved) {
        ...
        cbm_daemon_runtime_application_status_t status =
            cbm_daemon_runtime_client_application_request_tagged(
                client, second_token, second_request,
                (uint32_t)sizeof(second_request), &response, &response_length,
                RUNTIME_TEST_TIMEOUT_MS);
        second_exact = status == CBM_DAEMON_RUNTIME_APPLICATION_OK &&
                       response_length == sizeof(second_request) && response &&
                       memcmp(response, second_request,
                              sizeof(second_request)) == 0;
        free(response);
    }
    if (client) {
        closed = cbm_daemon_runtime_client_close(
...
TEST(daemon_runtime_consumes_busy_application_token_before_response) {
    ...
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start_application(
        &fixture, "application-token-replay", &identity, &context);
    cbm_daemon_ipc_connection_t *raw = NULL;
    ...
    if (raw) {
        first_sent = runtime_test_raw_application_send_token(
            raw, FIRST_TOKEN, blocking_request,
            (uint32_t)sizeof(blocking_request),
            (uint32_t)sizeof(blocking_request));
        first_started = first_sent && runtime_test_wait_atomic_bool(
                                          &context.first_request_started,
                                          RUNTIME_TEST_TIMEOUT_MS);
    }
    if (first_started) {
        busy_sent = runtime_test_raw_application_send_token(
            raw, BUSY_TOKEN, busy_request, (uint32_t)sizeof(busy_request),
            (uint32_t)sizeof(busy_request));
        busy_received =
            busy_sent && runtime_test_raw_application_receive_status_token(
                             raw, BUSY_TOKEN,
                             CBM_DAEMON_RUNTIME_APPLICATION_BUSY);
    }
    if (busy_received) {
    ...
    if (cancellation_received) {
        replay_sent = runtime_test_raw_application_send_token(
            raw, BUSY_TOKEN, busy_request, (uint32_t)sizeof(busy_request),
            (uint32_t)sizeof(busy_request));
    }
    if (replay_sent) {
        ...
        uint8_t *payload = NULL;
        int received = cbm_daemon_ipc_receive_frame(
            raw, RUNTIME_TEST_TIMEOUT_MS, &frame, &payload);
...
TEST(daemon_runtime_close_begin_retains_storage_and_rejects_late_exchange) {
    ...
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start_application(
        &fixture, "application-close-gap", &identity, &context);
    cbm_daemon_runtime_connect_result_t result = {0};
    cbm_daemon_runtime_client_t *client = NULL;
    uint8_t *response = NULL;
    uint32_t response_length = 0;
    ...
    if (close_begun) {
        ...
        status = cbm_daemon_runtime_client_application_request(
            client, request, (uint32_t)sizeof(request), &response,
            &response_length, RUNTIME_TEST_TIMEOUT_MS);
        close_acknowledged = cbm_daemon_runtime_client_close_finish(
            client, RUNTIME_TEST_TIMEOUT_MS);
        client = NULL;
        exited = cbm_daemon_runtime_service_wait_exited(
            fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    ...
    if (client) {
        if (close_begun) {
            (void)cbm_daemon_runtime_client_close_finish(
                client, RUNTIME_TEST_TIMEOUT_MS);
        } else {
            (void)cbm_daemon_runtime_client_close(client,
                                                  RUNTIME_TEST_TIMEOUT_MS);
...
TEST(daemon_runtime_disconnect_cancels_blocked_application_before_exit) {
    ...
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start_application(
        &fixture, "application-cancel", &identity, &context);
    cbm_daemon_runtime_client_t *client = NULL;
    cbm_daemon_runtime_connect_result_t result = {0};
    runtime_application_client_call_t call = {
        .request = request,
    ...
    if (client) {
        close_acknowledged = close_begun
                                 ? cbm_daemon_runtime_client_close_finish(
                                       client, RUNTIME_TEST_TIMEOUT_MS)
                                 : cbm_daemon_runtime_client_close(
                                       client, RUNTIME_TEST_TIMEOUT_MS);
...
TEST(daemon_runtime_noncooperative_callback_does_not_detach_or_unbound_stop) {
    ...
    runtime_application_context_t context;
    runtime_application_context_init(&context, true);
    atomic_store_explicit(&context.ignore_first_request_cancel, true,
                          memory_order_release);
    atomic_bool request_thread_completed;
    atomic_init(&request_thread_completed, false);
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start_application(
        &fixture, "application-noncooperative", &identity, &context);
    cbm_daemon_runtime_connect_result_t result = {0};
    cbm_daemon_runtime_client_t *client = NULL;
    runtime_application_client_call_t call = {
        .request = request,
    ...
    if (callback_started) {
        ...
        stop_returned = cbm_daemon_runtime_service_stop(
            fixture.service, STOP_BOUND_MS);
    ...
    atomic_store_explicit(&context.release_first_request, true,
                          memory_order_release);
    if (request_thread_started) {
        request_thread_join_rc = cbm_thread_join(&request_thread);
    ...
    if (started) {
        exited_after_release = cbm_daemon_runtime_service_wait_exited(
            fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
    free(call.response);
    runtime_test_fixture_finish(&fixture);
...
TEST(daemon_runtime_application_busy_cap_and_malformed_are_isolated) {
    ...
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start_application(
        &fixture, "application-isolation", &identity, &context);
    cbm_daemon_runtime_client_t *owner = NULL;
    cbm_daemon_runtime_connect_result_t owner_result = {0};
    cbm_daemon_ipc_connection_t *raw = NULL;
    bool first_sent = false;
    ...
    cbm_daemon_runtime_application_status_t oversize_status =
        CBM_DAEMON_RUNTIME_APPLICATION_OK;
    ...
    if (owner && raw) {
        first_sent = runtime_test_raw_application_send(
            raw, blocking_request, (uint32_t)sizeof(blocking_request),
            (uint32_t)sizeof(blocking_request));
        callback_started = first_sent && runtime_test_wait_atomic_bool(
                                             &context.first_request_started,
                                             RUNTIME_TEST_TIMEOUT_MS);
    }
    if (callback_started) {
        busy_sent = runtime_test_raw_application_send(
            raw, busy_request, (uint32_t)sizeof(busy_request),
            (uint32_t)sizeof(busy_request));
        busy_rejected = busy_sent &&
                        runtime_test_raw_application_receive_status(
    ...
    if (owner && bad_peer_released) {
        oversize_status = cbm_daemon_runtime_client_application_request(
            owner, &sentinel,
            CBM_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX + 1U,
            &oversize_response, &oversize_response_length,
            RUNTIME_TEST_TIMEOUT_MS);
        valid_status = cbm_daemon_runtime_client_application_request(
            owner, valid_request, (uint32_t)sizeof(valid_request),
            &valid_response, &valid_response_length,
            RUNTIME_TEST_TIMEOUT_MS);
        owner_survived =
            oversize_status ==
                CBM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR &&
            ...
            cbm_daemon_runtime_client_heartbeat(owner,
                                                RUNTIME_TEST_TIMEOUT_MS);
        free(valid_response);
        valid_response = NULL;
        (void)cbm_daemon_runtime_client_close(owner,
...
TEST(daemon_runtime_malformed_and_zero_cancel_close_only_offending_connections) {
    ...
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start_application(
        &fixture, "application-cancel-isolation", &identity, &context);
    cbm_daemon_runtime_connect_result_t owner_result = {0};
    cbm_daemon_runtime_client_t *owner = NULL;
    cbm_daemon_ipc_connection_t *raw = NULL;
    ...
    if (raw) {
        malformed_sent = cbm_daemon_ipc_send_frame(
            raw, CBM_DAEMON_FRAME_REQUEST,
            CBM_DAEMON_RUNTIME_OP_APPLICATION_CANCEL, malformed_cancel,
            (uint32_t)sizeof(malformed_cancel));
    }
    if (malformed_sent) {
    ...
    if (raw) {
        zero_sent = cbm_daemon_ipc_send_frame(
            raw, CBM_DAEMON_FRAME_REQUEST,
            CBM_DAEMON_RUNTIME_OP_APPLICATION_CANCEL, zero_token_cancel,
            (uint32_t)sizeof(zero_token_cancel));
    }
    if (zero_sent) {
        zero_released = cbm_daemon_runtime_service_wait_for_clients(
            fixture.service, 1, RUNTIME_TEST_TIMEOUT_MS);
    }
    if (zero_released) {
        cbm_daemon_frame_t frame = {0};
...
TEST(daemon_runtime_kernel_process_fingerprint_is_stable_and_fail_closed) {
    ...
    bool first_ok = cbm_daemon_runtime_process_build_fingerprint(
        runtime_test_process_id(), first);
    bool repeated_ok = cbm_daemon_runtime_process_build_fingerprint(
        runtime_test_process_id(), repeated);
    bool invalid_rejected = !cbm_daemon_runtime_process_build_fingerprint(
        UINT64_MAX, invalid);
...
TEST(daemon_runtime_mac_fast_path_rejects_foreign_main_image_mapping_active) {
    ...
    int directory_written = snprintf(
        directory, sizeof(directory), "%s/cbm-runtime-mapped-image-XXXXXX",
        cbm_tmpdir());
    bool directory_created =
        directory_written > 0 && directory_written < (int)sizeof(directory) &&
        cbm_mkdtemp(directory) != NULL;
    int foreign_written =
        directory_created
            ? snprintf(foreign_image, sizeof(foreign_image), "%s/foreign-client",
                       directory)
            : -1;
    bool active_resolved = runtime_test_self_image_path(active_image);
    bool foreign_copied =
        active_resolved && foreign_written > 0 &&
    ...
    bool foreign_signed =
        foreign_changed && runtime_test_mac_ad_hoc_sign(foreign_image);
    bool foreign_fingerprinted =
        foreign_signed && cbm_daemon_build_fingerprint_file(
                              foreign_image, foreign_fingerprint);
    bool fingerprint_differs =
        foreign_fingerprinted &&
        strcmp(foreign_fingerprint, identity.build_fingerprint) != 0;

    runtime_test_fixture_t fixture;
    memset(&fixture, 0, sizeof(fixture));
    bool fixture_attempted = directory_created && active_resolved &&
                             fingerprint_differs;
    bool started = fixture_attempted && runtime_test_fixture_start(
                                          &fixture, "mapped-foreign", &identity);
    int foreign_exit = -1;
    bool foreign_ran =
        started && runtime_test_run_mapped_hello_image(
                       foreign_image, active_image, &fixture, &identity,
                       &foreign_exit);

    if (fixture_attempted) {
        runtime_test_fixture_finish(&fixture);
    }
    if (foreign_written > 0 &&
        foreign_written < (int)sizeof(foreign_image)) {
        (void)cbm_unlink(foreign_image);
    }
    if (directory_created) {
...
TEST(daemon_runtime_copied_image_fallback_accepts_identical_and_rejects_changed) {
    ...
    bool identical_started = runtime_test_fixture_start(
        &identical_fixture, "copied-identical", &identity);
    char identical_path[RUNTIME_TEST_PATH_CAP] = {0};
    int identical_path_written =
        identical_started
#ifdef _WIN32
            ? snprintf(identical_path, sizeof(identical_path),
                       "%s/client-copy.exe", identical_fixture.parent)
#else
            ? snprintf(identical_path, sizeof(identical_path), "%s/client-copy",
                       identical_fixture.parent)
#endif
            : -1;
    ...
    bool identical_bytes =
        identical_copied && cbm_daemon_build_fingerprint_file(
                                identical_path, identical_fingerprint) &&
        strcmp(identical_fingerprint, identity.build_fingerprint) == 0;
    int identical_exit = -1;
    bool identical_ran = identical_bytes && runtime_test_run_hello_image(
                                                identical_path,
                                                &identical_fixture, &identity,
                                                &identical_exit);
    bool identical_exited =
        identical_ran && identical_exit == 0 &&
        cbm_daemon_runtime_service_wait_exited(
            identical_fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    (void)cbm_unlink(identical_path);
    runtime_test_fixture_finish(&identical_fixture);

#ifdef __linux__
    runtime_test_fixture_t changed_fixture;
    bool changed_started = runtime_test_fixture_start(
        &changed_fixture, "copied-changed", &identity);
    char changed_path[RUNTIME_TEST_PATH_CAP] = {0};
    int changed_path_written =
        changed_started
            ? snprintf(changed_path, sizeof(changed_path), "%s/client-copy",
                       changed_fixture.parent)
            : -1;
    bool changed_copied =
        changed_path_written > 0 &&
    ...
    bool changed_bytes =
        changed_copied && cbm_daemon_build_fingerprint_file(
                              changed_path, changed_fingerprint) &&
        strcmp(changed_fingerprint, identity.build_fingerprint) != 0;
    int changed_exit = -1;
    bool changed_ran = changed_bytes && runtime_test_run_hello_image(
                                            changed_path, &changed_fixture,
...
TEST(daemon_runtime_process_fingerprint_never_hashes_replacement_path) {
    ...
    int directory_written = snprintf(directory, sizeof(directory),
                                     "%s/cbm-runtime-image-XXXXXX",
                                     cbm_tmpdir());
    bool setup = directory_written > 0 &&
                 directory_written < (int)sizeof(directory) &&
                 cbm_mkdtemp(directory) != NULL;
    int image_written =
        setup ? snprintf(image_path, sizeof(image_path), "%s/image.exe",
                         directory)
              : -1;
    int replacement_written =
        setup ? snprintf(replacement_path, sizeof(replacement_path),
                         "%s/replacement.exe", directory)
              : -1;
    int event_written = snprintf(
        event_name, sizeof(event_name), "Local\\cbm-runtime-image-%lu-%llu",
        (unsigned long)GetCurrentProcessId(),
        (unsigned long long)GetTickCount64());
    setup = setup && image_written > 0 &&
            image_written < (int)sizeof(image_path) &&
    ...
    bool replacement_written_ok =
        replacement_file &&
        fputs("cbm-windows-replacement-image", replacement_file) >= 0;
    if (replacement_file) {
        replacement_written_ok = fclose(replacement_file) == 0 &&
                                 replacement_written_ok;
    }
    ...
    HANDLE ready_event =
        setup ? CreateEventA(NULL, TRUE, FALSE, event_name) : NULL;
    bool event_private = ready_event && GetLastError() != ERROR_ALREADY_EXISTS;
    PROCESS_INFORMATION process;
    memset(&process, 0, sizeof(process));
    bool spawned = event_private && runtime_test_windows_spawn_image_holder(
                                        image_path, event_name, &process);
    bool ready = spawned &&
                 WaitForSingleObject(ready_event, 5000) == WAIT_OBJECT_0;
    bool replaced = ready && runtime_test_windows_posix_replace(
                                 replacement_path, image_path);
    bool fingerprinted =
        ready && cbm_daemon_runtime_process_build_fingerprint(
                     (uint64_t)process.dwProcessId, observed);
    bool replacement_safe =
        replaced ? (!fingerprinted || strcmp(observed, original) == 0)
                 : (fingerprinted && strcmp(observed, original) == 0);

    bool stopped = !spawned;
    if (spawned) {
        DWORD exit_code = 0;
        bool running = GetExitCodeProcess(process.hProcess, &exit_code) != 0 &&
                       exit_code == STILL_ACTIVE;
        bool termination_requested =
            !running || TerminateProcess(process.hProcess, 26) != 0;
...
TEST(daemon_runtime_process_fingerprint_never_hashes_replacement_path) {
    ...
    bool setup = directory_written > 0 &&
                 directory_written < (int)sizeof(directory) &&
                 cbm_mkdtemp(directory) != NULL;
    int image_written = setup
                            ? snprintf(image_path, sizeof(image_path), "%s/image",
                                       directory)
                            : -1;
    int replacement_written = setup
                                  ? snprintf(replacement_path,
    ...
    int release_fd = -1;
    pid_t child = setup
                      ? runtime_test_spawn_blocked_executable(image_path,
                                                              &release_fd)
                      : -1;
    setup = setup && child > 0 && rename(replacement_path, image_path) == 0 &&
            ...
            strcmp(original, replacement) != 0;
    bool fingerprinted =
        setup && cbm_daemon_runtime_process_build_fingerprint((uint64_t)child,
                                                              observed);
