...
static void project_lock_test_release(cbm_project_lock_lease_t **lease) {
    while (lease && *lease &&
           cbm_project_lock_lease_release(lease) != CBM_PRIVATE_FILE_LOCK_OK) {
        cbm_usleep(1000);
    }
}
...
TEST(project_lock_coordinates_instances_projects_wildcard_and_case_aliases) {
    ...
    (void)snprintf(runtime_parent, sizeof(runtime_parent),
                   "%s/cbm-project-lock-XXXXXX", cbm_tmpdir());
    ...
    cbm_project_lock_manager_t *first =
        cbm_project_lock_manager_new(endpoint);
    cbm_project_lock_manager_t *second =
        cbm_project_lock_manager_new(endpoint);
    ...
    ASSERT_EQ(cbm_project_lock_try_acquire(first, "Foo", &foo),
              CBM_PRIVATE_FILE_LOCK_OK);
    ASSERT_NOT_NULL(foo);
    ASSERT_EQ(cbm_project_lock_try_acquire(second, "foo", &alias),
    ...
    ASSERT_EQ(cbm_project_lock_acquire(first, "*", UINT64_MAX, NULL, &all),
              CBM_PRIVATE_FILE_LOCK_OK);
    ASSERT_NOT_NULL(all);
    ASSERT_EQ(cbm_project_lock_try_acquire(second, "unrelated", &bar),
              CBM_PRIVATE_FILE_LOCK_BUSY);
    ASSERT_NULL(bar);
