...
static const char VERSION_COHORT_ADMISSION_FILE[] =
    "cbm-version-cohort-admission-v1.lock";
static const char VERSION_COHORT_LIFETIME_FILE[] =
    "cbm-version-cohort-lifetime-v1.lock";
static const char VERSION_COHORT_MAINTENANCE_FILE[] =
    "cbm-version-cohort-maintenance-v1.lock";
static const char VERSION_COHORT_DAEMON_FILE[] =
...
static cbm_private_file_lock_status_t version_cohort_lock_release_until(
    ...
    while (*lock_io) {
        ...
        if (cbm_now_ms() >= deadline_ms) {
            return result == CBM_PRIVATE_FILE_LOCK_OK
...
static _Noreturn void version_cohort_cleanup_fail_stop(
...
#ifndef _WIN32
static void version_cohort_startup_lock_release_complete(
    cbm_daemon_ipc_startup_lock_t **lock_io) {
    uint64_t deadline = version_cohort_deadline_after(
...
static bool version_cohort_identity_valid(
    const cbm_daemon_build_identity_t *identity) {
    cbm_daemon_conflict_t comparison;
    return cbm_daemon_hello_compare(identity, identity, &comparison) ==
...
static bool version_cohort_record_encode(
    const cbm_daemon_build_identity_t *identity,
    ...
    memcpy(out, VERSION_COHORT_RECORD_MAGIC,
           VERSION_COHORT_RECORD_MAGIC_SIZE);
    size_t version_length = strlen(identity->semantic_version);
    size_t build_length = strlen(identity->build_fingerprint);
    memcpy(out + VERSION_COHORT_RECORD_VERSION_OFFSET,
           identity->semantic_version, version_length + 1);
    memcpy(out + VERSION_COHORT_RECORD_BUILD_OFFSET,
           identity->build_fingerprint, build_length + 1);
    version_cohort_u32_encode(out + VERSION_COHORT_RECORD_PROTOCOL_OFFSET,
                              identity->protocol_abi);
    version_cohort_u32_encode(out + VERSION_COHORT_RECORD_STORE_OFFSET,
                              identity->store_abi);
    version_cohort_u32_encode(out + VERSION_COHORT_RECORD_FEATURE_OFFSET,
                              identity->feature_abi);
    return true;
}

static bool version_cohort_record_decode(
    const unsigned char record[VERSION_COHORT_RECORD_SIZE], size_t length,
    cbm_daemon_build_identity_t *identity_out,
    char version_out[CBM_DAEMON_VERSION_TEXT_SIZE],
    char build_out[CBM_DAEMON_BUILD_FINGERPRINT_SIZE]) {
    if (!record || length != VERSION_COHORT_RECORD_SIZE || !identity_out ||
        !version_out || !build_out ||
        memcmp(record, VERSION_COHORT_RECORD_MAGIC,
               VERSION_COHORT_RECORD_MAGIC_SIZE) != 0) {
        return false;
    }
    ...
    cbm_daemon_build_identity_t identity = {
        ...
        .protocol_abi = version_cohort_u32_decode(
            record + VERSION_COHORT_RECORD_PROTOCOL_OFFSET),
        .store_abi = version_cohort_u32_decode(
            record + VERSION_COHORT_RECORD_STORE_OFFSET),
        .feature_abi = version_cohort_u32_decode(
            record + VERSION_COHORT_RECORD_FEATURE_OFFSET),
    };
...
static cbm_private_file_lock_status_t version_cohort_lock_until(
    cbm_version_cohort_manager_t *manager, const char *base_name,
    cbm_private_file_lock_mode_t mode, uint64_t deadline_ms,
    cbm_private_file_lock_t **lock_out) {
    for (;;) {
        cbm_private_file_lock_status_t status =
            cbm_private_file_lock_try_acquire(manager->directory, base_name,
                                              mode, lock_out);
...
static cbm_version_cohort_lease_t *version_cohort_lease_new(
    cbm_version_cohort_manager_t *manager) {
    if (!manager) {
        return NULL;
    }
...
static bool version_cohort_manager_register(
    cbm_version_cohort_manager_t *manager) {
    if (!manager) {
        return false;
    }
...
static bool version_cohort_manager_unregister(
    cbm_version_cohort_manager_t *manager) {
    if (!manager) {
        return false;
    }
    cbm_mutex_lock(&manager->mutex);
    bool registered = manager->owner_pid == version_cohort_current_pid() &&
                      manager->lease_count > 0;
    if (registered) {
        manager->lease_count--;
    }
    cbm_mutex_unlock(&manager->mutex);
...
static cbm_version_cohort_status_t version_cohort_failed(
    cbm_version_cohort_lease_t *lease, cbm_version_cohort_status_t status,
    cbm_version_cohort_lease_t **lease_out) {
    cbm_version_cohort_lease_t *cleanup = lease;
    cbm_private_file_lock_status_t cleanup_status =
        cbm_version_cohort_lease_release(&cleanup);
    if (cleanup) {
        *lease_out = cleanup;
        return CBM_VERSION_COHORT_IO;
    }
    return cleanup_status == CBM_PRIVATE_FILE_LOCK_OK ? status
...
static cbm_version_cohort_status_t version_cohort_claim_new(
    ...
    if (!version_cohort_record_encode(identity, record) ||
        cbm_private_file_lock_payload_write(lease->lifetime, record,
                                            sizeof(record)) !=
    ...
    return version_cohort_status_from_lock(version_cohort_lock_until(
        lease->manager, VERSION_COHORT_LIFETIME_FILE,
...
cbm_version_cohort_status_t cbm_version_cohort_acquire(
    ...
    cbm_private_file_lock_status_t lock_status =
        cbm_private_file_lock_try_acquire(
            manager->directory, VERSION_COHORT_MAINTENANCE_FILE,
            CBM_PRIVATE_FILE_LOCK_SH, &lease->maintenance);
    if (lock_status != CBM_PRIVATE_FILE_LOCK_OK) {
        return version_cohort_failed(
            lease, version_cohort_status_from_lock(lock_status), lease_out);
    }
    lock_status = version_cohort_lock_until(
        manager, VERSION_COHORT_ADMISSION_FILE, CBM_PRIVATE_FILE_LOCK_EX,
    ...
    if (status == CBM_VERSION_COHORT_OK) {
        ...
        if (status == CBM_VERSION_COHORT_OK) {
            ...
            if (cbm_private_file_lock_payload_read(
                    lease->lifetime, record, sizeof(record),
                    &record_length) != CBM_PRIVATE_FILE_LOCK_OK ||
                !version_cohort_record_decode(
                    record, record_length, &active, active_version,
                    active_build)) {
                ...
                if (comparison == CBM_DAEMON_HELLO_COMPATIBLE) {
                    ...
                    if (release_status != CBM_PRIVATE_FILE_LOCK_OK) {
                        ...
                        lock_status = cbm_private_file_lock_try_acquire(
                            manager->directory,
                            VERSION_COHORT_LIFETIME_FILE,
                            CBM_PRIVATE_FILE_LOCK_EX, &lease->lifetime);
                        status = version_cohort_status_from_lock(lock_status);
                        if (status == CBM_VERSION_COHORT_OK) {
                            status = version_cohort_claim_new(
                                lease, identity, deadline_ms);
                        } else if (status == CBM_VERSION_COHORT_BUSY) {
    ...
    if (admission_release != CBM_PRIVATE_FILE_LOCK_OK) {
        return version_cohort_failed(lease, CBM_VERSION_COHORT_IO,
                                     lease_out);
    }
...
static cbm_version_cohort_status_t
version_cohort_reserve_for_mutation_internal(
    ...
    cbm_version_cohort_lease_t **lease_out, bool require_finite_deadline) {
    if (lease_out) {
        *lease_out = NULL;
    }
    ...
    if (lock_status == CBM_PRIVATE_FILE_LOCK_BUSY) {
        if (!quiesce) {
            *quiesce_result_out = CBM_VERSION_COHORT_QUIESCE_REFUSED;
            return version_cohort_failed(lease, CBM_VERSION_COHORT_BUSY,
                                         lease_out);
        }
        ...
        *quiesce_result_out = quiesce_result;
        ...
        if (quiesce_result != CBM_VERSION_COHORT_QUIESCE_REQUESTED) {
            return version_cohort_failed(lease, CBM_VERSION_COHORT_UNSAFE,
                                         lease_out);
        }
        lock_status = version_cohort_lock_until(
            manager, VERSION_COHORT_LIFETIME_FILE,
...
cbm_version_cohort_status_t cbm_version_cohort_reserve_for_mutation(
    ...
    return version_cohort_reserve_for_mutation_internal(
        manager, deadline_ms, quiesce, quiesce_context, quiesce_result_out,
        lease_out, true);
}
...
cbm_version_cohort_status_t cbm_version_cohort_reserve_exclusive(
    ...
    return version_cohort_reserve_for_mutation_internal(
        manager, deadline_ms, NULL, NULL, &ignored_quiesce, lease_out, false);
}
...
cbm_version_cohort_maintenance_presence(
    ...
    cbm_private_file_lock_status_t status =
        cbm_private_file_lock_try_acquire(
            manager->directory, VERSION_COHORT_MAINTENANCE_FILE,
            CBM_PRIVATE_FILE_LOCK_SH, &observer);
    ...
    cbm_private_file_lock_status_t observer_release =
        version_cohort_lock_release_until(
            &observer, version_cohort_deadline_after(
                           VERSION_COHORT_CLEANUP_TIMEOUT_MS));
...
cbm_version_cohort_status_t cbm_version_cohort_daemon_claim_acquire(
    ...
    cbm_private_file_lock_status_t lock_status =
        cbm_private_file_lock_try_acquire(
            manager->directory, VERSION_COHORT_DAEMON_FILE,
            CBM_PRIVATE_FILE_LOCK_EX, &claim->marker);
    if (lock_status == CBM_PRIVATE_FILE_LOCK_OK) {
        *claim_out = claim;
        return CBM_VERSION_COHORT_OK;
    }
    ...
    cbm_private_file_lock_status_t cleanup_status =
        version_cohort_lock_release_until(
            &claim->marker, version_cohort_deadline_after(
                                VERSION_COHORT_CLEANUP_TIMEOUT_MS));
    if (claim->marker) {
        *claim_out = claim;
        return CBM_VERSION_COHORT_IO;
...
cbm_version_cohort_daemon_claim_presence(
    ...
    cbm_version_cohort_daemon_presence_t presence =
        CBM_VERSION_COHORT_DAEMON_IO;
    cbm_private_file_lock_t *marker = NULL;
    cbm_private_file_lock_status_t marker_status =
        cbm_private_file_lock_try_acquire(
            manager->directory, VERSION_COHORT_DAEMON_FILE,
            CBM_PRIVATE_FILE_LOCK_EX, &marker);
    ...
    cbm_private_file_lock_status_t marker_release =
        version_cohort_lock_release_until(
            &marker, version_cohort_deadline_after(
                         VERSION_COHORT_CLEANUP_TIMEOUT_MS));
...
static cbm_version_cohort_daemon_presence_t
version_cohort_active_daemon_presence(
    cbm_version_cohort_manager_t *manager,
    const cbm_daemon_ipc_endpoint_t *endpoint,
...
cbm_version_cohort_daemon_presence_t cbm_version_cohort_daemon_presence(
    ...
    cbm_version_cohort_daemon_presence_t claim =
        cbm_version_cohort_daemon_claim_presence(manager);
    if (claim == CBM_VERSION_COHORT_DAEMON_COORDINATED) {
        ...
        if (lifetime == 0) {
            ...
            if (startup_status == 0) {
                ...
                int cleanup = cbm_daemon_ipc_stale_generation_cleanup(
                    endpoint, startup);
                if (cleanup == 1) {
                    presence = CBM_VERSION_COHORT_DAEMON_ABSENT;
                } else if (cleanup == 0) {
            ...
            presence = version_cohort_active_daemon_presence(manager, NULL,
                                                             NULL);
        }
    }
    if (!version_cohort_manager_unregister(manager)) {
...
cbm_version_cohort_daemon_presence_under_transition(
    ...
    const cbm_daemon_ipc_endpoint_t *endpoint,
    const cbm_daemon_ipc_local_transition_t *transition) {
    if (!manager || !endpoint || !transition ||
        !version_cohort_manager_register(manager)) {
        return CBM_VERSION_COHORT_DAEMON_UNSAFE;
    }
...
cbm_private_file_lock_status_t cbm_version_cohort_manager_free(
    ...
    bool can_free = manager->owner_pid == version_cohort_current_pid() &&
                    !leases_active && !manager->closing;
    if (can_free) {
        manager->closing = true;
    }
    cbm_mutex_unlock(&manager->mutex);
    if (!can_free) {
        return leases_active ? CBM_PRIVATE_FILE_LOCK_BUSY
                             : CBM_PRIVATE_FILE_LOCK_IO;
    }
...
bool cbm_version_cohort_log_conflict(const cbm_daemon_conflict_t *conflict) {
    ...
    if (logs_written <= 0 || logs_written >= (int)sizeof(logs) ||
        path_written <= 0 || path_written >= (int)sizeof(path)) {
        return false;
    }
    FILE *seed = cbm_daemon_ipc_private_log_open(
        logs, "daemon-conflicts.ndjson", VERSION_COHORT_LOG_CAP);
    if (!seed) {
        return false;
    }
    bool seeded = fclose(seed) == 0;
    return seeded && cbm_daemon_conflict_log_append(
                         path, conflict, VERSION_COHORT_LOG_CAP);
}
...
bool cbm_version_cohort_log_uncoordinated_daemon(
    ...
    (void)snprintf(conflict.active_version,
                   ...
                   "%s", "pre-cohort/unknown");
    memset(conflict.active_build_fingerprint, '0',
           sizeof(conflict.active_build_fingerprint) - 1);
    conflict.active_build_fingerprint[
        sizeof(conflict.active_build_fingerprint) - 1] = '\0';
    (void)snprintf(conflict.requested_version,
                   sizeof(conflict.requested_version), "%s",
