...
_Static_assert(CBM_DAEMON_HELLO_INVALID == 0 &&
                   CBM_DAEMON_HELLO_COMPATIBLE == 1 &&
                   CBM_DAEMON_HELLO_VERSION_CONFLICT == 2 &&
                   CBM_DAEMON_HELLO_BUILD_CONFLICT == 3 &&
...
_Static_assert(CBM_DAEMON_RENDEZVOUS_FRAME_VERSION == 1 &&
                   CBM_DAEMON_RENDEZVOUS_ABI == 1 &&
...
static void runtime_wait_tick(uint64_t deadline_ms) {
    ...
    struct timespec pause = {
        .tv_sec = 0,
        .tv_nsec = remaining_ms > 1 ? RUNTIME_WAIT_POLL_NS
...
static bool runtime_bounded_length(const char *value, size_t capacity,
                                   size_t *length_out) {
    if (!value || capacity == 0) {
        return false;
    }
...
static bool runtime_wire_string_encode(uint8_t *out, size_t capacity,
                                       const char *value, bool allow_empty) {
    size_t length = 0;
...
static cbm_daemon_hello_status_t runtime_rendezvous_compare(
    const char *active_version, const char *active_build,
    const char *requested_version, const char *requested_build,
    ...
    cbm_daemon_hello_status_t status =
        cbm_daemon_hello_compare(&active, &requested, conflict_out);
    if (status != CBM_DAEMON_HELLO_INVALID &&
        status != CBM_DAEMON_HELLO_COMPATIBLE &&
...
bool cbm_daemon_runtime_hello_request_encode(
    ...
    runtime_put_u32(out + RENDEZVOUS_REQUEST_ABI_OFFSET,
                    CBM_DAEMON_RENDEZVOUS_ABI);
    return runtime_wire_string_encode(
               out + RENDEZVOUS_REQUEST_VERSION_OFFSET,
               CBM_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
               identity->semantic_version, false) &&
           runtime_wire_string_encode(
               out + RENDEZVOUS_REQUEST_BUILD_OFFSET,
               CBM_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP,
...
static bool runtime_rendezvous_request_decode(
    ...
    if (!payload || !version || !build ||
        runtime_get_u32(payload + RENDEZVOUS_REQUEST_ABI_OFFSET) !=
            CBM_DAEMON_RENDEZVOUS_ABI ||
        !runtime_wire_string_decode(
            payload + RENDEZVOUS_REQUEST_VERSION_OFFSET,
            CBM_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP, version, false) ||
        !runtime_wire_string_decode(
            payload + RENDEZVOUS_REQUEST_BUILD_OFFSET,
            CBM_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP, build, false)) {
        return false;
    }
    cbm_daemon_conflict_t validation;
    return runtime_rendezvous_compare(version, build, version, build,
                                      &validation) ==
           CBM_DAEMON_HELLO_COMPATIBLE;
}

static bool runtime_activation_action_valid(
...
static const char *runtime_activation_action_text(
    cbm_daemon_runtime_activation_action_t action) {
    switch (action) {
...
static bool runtime_activation_request_encode(
    ...
    runtime_put_u32(out + ACTIVATION_REQUEST_ACTION_OFFSET,
                    (uint32_t)action);
    return cbm_daemon_runtime_hello_request_encode(
        out + ACTIVATION_REQUEST_IDENTITY_OFFSET, identity);
}

static bool runtime_activation_request_decode(
    const uint8_t payload[CBM_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE],
    cbm_daemon_runtime_activation_action_t *action_out,
    char version[CBM_DAEMON_VERSION_TEXT_SIZE],
    char build[CBM_DAEMON_BUILD_FINGERPRINT_SIZE]) {
    if (!payload || !action_out || !version || !build) {
        return false;
    }
    uint32_t raw_action = runtime_get_u32(
        payload + ACTIVATION_REQUEST_ACTION_OFFSET);
    if (raw_action < (uint32_t)CBM_DAEMON_RUNTIME_ACTIVATION_INSTALL ||
        raw_action > (uint32_t)CBM_DAEMON_RUNTIME_ACTIVATION_UNINSTALL ||
        !runtime_rendezvous_request_decode(
            payload + ACTIVATION_REQUEST_IDENTITY_OFFSET, version, build)) {
        return false;
    }
    cbm_daemon_runtime_activation_action_t action =
...
static bool runtime_activation_response_encode(
    ...
    runtime_put_u32(out + ACTIVATION_RESPONSE_ABI_OFFSET,
                    CBM_DAEMON_RENDEZVOUS_ABI);
    runtime_put_u32(out + ACTIVATION_RESPONSE_ACCEPTED_OFFSET,
                    result->accepted ? 1U : 0U);
    runtime_put_u64(out + ACTIVATION_RESPONSE_CLIENTS_OFFSET,
                    result->active_clients);
    runtime_put_u64(out + ACTIVATION_RESPONSE_CONNECTIONS_OFFSET,
...
static bool runtime_activation_response_decode(
    ...
    if (!payload || !result_out ||
        runtime_get_u32(payload + ACTIVATION_RESPONSE_ABI_OFFSET) !=
            CBM_DAEMON_RENDEZVOUS_ABI) {
        return false;
    }
    uint32_t accepted =
        runtime_get_u32(payload + ACTIVATION_RESPONSE_ACCEPTED_OFFSET);
    if (accepted > 1U) {
        return false;
    }
    result_out->accepted = accepted == 1U;
    result_out->active_clients =
        runtime_get_u64(payload + ACTIVATION_RESPONSE_CLIENTS_OFFSET);
...
static void runtime_process_image_reference_init(
...
static bool runtime_process_image_reference_release(
    runtime_process_image_reference_t *reference) {
    if (!reference) {
        return false;
    }
    bool ok = true;
#ifdef _WIN32
    if (reference->file != INVALID_HANDLE_VALUE &&
        !CloseHandle(reference->file)) {
        ok = false;
    }
#elif defined(__APPLE__) || defined(__linux__)
...
static bool runtime_windows_process_image_snapshot(
    ...
    FILETIME user_time;
    DWORD exit_code = 0;
    DWORD path_length = (DWORD)(sizeof(snapshot->path) /
                               sizeof(snapshot->path[0]));
    bool ok = GetProcessTimes(process, &snapshot->creation_time, &exit_time,
                              &kernel_time, &user_time) != 0 &&
              GetExitCodeProcess(process, &exit_code) != 0 &&
              exit_code == STILL_ACTIVE &&
              QueryFullProcessImageNameW(process, 0, snapshot->path,
                                         &path_length) != 0 &&
              path_length > 0 &&
              path_length < (DWORD)(sizeof(snapshot->path) /
                                    sizeof(snapshot->path[0]));
    if (ok) {
...
static bool runtime_windows_process_image_snapshot_same(
    ...
    return first && second &&
           CompareFileTime(&first->creation_time, &second->creation_time) == 0 &&
           CompareStringOrdinal(first->path, -1, second->path, -1, TRUE) ==
               CSTR_EQUAL;
}

static bool runtime_windows_file_snapshot(
    HANDLE file, BY_HANDLE_FILE_INFORMATION *information, LARGE_INTEGER *size) {
    if (!information || !size) {
        return false;
    }
...
static bool runtime_windows_file_snapshot_same(
    const BY_HANDLE_FILE_INFORMATION *first_information,
    ...
    return first_information && first_size && second_information && second_size &&
           first_information->dwVolumeSerialNumber ==
               second_information->dwVolumeSerialNumber &&
...
static bool runtime_mac_process_instance(int process_id,
...
static bool runtime_mac_stat_matches_vnode(
    const struct stat *status, const struct vinfo_stat *vnode) {
    return status && vnode && S_ISREG(vnode->vst_mode) &&
           vnode->vst_dev == (uint32_t)status->st_dev &&
           vnode->vst_ino == (uint64_t)status->st_ino &&
           vnode->vst_size == status->st_size &&
...
static bool runtime_mac_stat_same(const struct stat *first,
                                  const struct stat *second) {
    return first && second && first->st_dev == second->st_dev &&
...
static bool runtime_mac_process_maps_file_executable(int process_id,
                                                     const struct stat *status) {
    uint64_t address = 0;
    for (size_t region_count = 0; region_count < 65536; region_count++) {
        struct proc_regionwithpathinfo region;
        memset(&region, 0, sizeof(region));
        int received = proc_pidinfo(process_id, PROC_PIDREGIONPATHINFO,
                                    address, &region, (int)sizeof(region));
        if (received != (int)sizeof(region)) {
            return false;
        }
        if ((region.prp_prinfo.pri_protection & VM_PROT_EXECUTE) != 0 &&
            runtime_mac_stat_matches_vnode(
...
static bool runtime_linux_stat_same_image(const struct stat *first,
                                          const struct stat *second) {
...
static bool runtime_process_image_reference_acquire(
    ...
    bool ok =
        runtime_windows_process_image_snapshot(process, &process_before);
    ...
    ok = ok && runtime_windows_file_snapshot(file, &file_before, &size_before) &&
         (!fingerprint || cbm_daemon_build_fingerprint_native_file(
                              (uintptr_t)file, fingerprint)) &&
         runtime_windows_file_snapshot(file, &file_after, &size_after) &&
         runtime_windows_file_snapshot_same(&file_before, &size_before,
                                            &file_after, &size_after) &&
         runtime_windows_process_image_snapshot(process, &process_after) &&
         runtime_windows_process_image_snapshot_same(&process_before,
                                                     &process_after);
    ...
    int fd = ok ? open(path, O_RDONLY | O_CLOEXEC | O_NOFOLLOW | O_NONBLOCK)
                : -1;
    struct stat file_before;
    struct stat file_after;
    ok = ok && fd >= 0 && fstat(fd, &file_before) == 0 &&
         S_ISREG(file_before.st_mode) &&
         runtime_mac_process_maps_file_executable(pid, &file_before) &&
         (!fingerprint || cbm_daemon_build_fingerprint_native_file(
                              (uintptr_t)fd, fingerprint)) &&
         fstat(fd, &file_after) == 0 &&
         runtime_mac_stat_same(&file_before, &file_after) &&
...
#elif defined(__linux__)
    char proc_path[64];
    int written = snprintf(proc_path, sizeof(proc_path), "/proc/%llu",
                           (unsigned long long)process_id);
    if (written <= 0 || written >= (int)sizeof(proc_path)) {
        return false;
    }
    int process_fd = open(proc_path, O_RDONLY | O_DIRECTORY | O_CLOEXEC |
                                        O_NOFOLLOW | O_NONBLOCK);
    int image_fd = process_fd >= 0
                       ? openat(process_fd, "exe", O_RDONLY | O_CLOEXEC)
                       : -1;
    ...
    bool ok = image_fd >= 0 && fstat(image_fd, &image_before) == 0 &&
              ...
              (!fingerprint || cbm_daemon_build_fingerprint_native_file(
                                   (uintptr_t)image_fd, fingerprint)) &&
...
static bool runtime_process_image_reference_matches_process(
    ...
    same = same &&
           runtime_windows_file_snapshot(active->file, &active_now,
                                         &active_size_now) &&
           runtime_windows_file_snapshot_same(
               &active->information, &active->size, &active_now,
               &active_size_now) &&
           runtime_windows_file_snapshot_same(
               &active->information, &active->size, &peer.information,
               &peer.size);
...
static bool runtime_hello_response_encode(
    uint8_t out[CBM_DAEMON_RENDEZVOUS_RESPONSE_SIZE],
    const cbm_daemon_runtime_connect_result_t *result) {
    if (!out || !result) {
        return false;
    }
    memset(out, 0, CBM_DAEMON_RENDEZVOUS_RESPONSE_SIZE);
    runtime_put_u32(out + RENDEZVOUS_RESPONSE_CONNECT_STATUS_OFFSET,
                    (uint32_t)result->status);
    runtime_put_u32(out + RENDEZVOUS_RESPONSE_HELLO_STATUS_OFFSET,
                    (uint32_t)result->hello_status);
    runtime_put_u64(out + RENDEZVOUS_RESPONSE_CLIENT_ID_OFFSET,
                    result->client_id);
    runtime_put_u64(out + RENDEZVOUS_RESPONSE_PROCESS_ID_OFFSET,
    ...
    if (!runtime_wire_string_encode(
            out + RENDEZVOUS_RESPONSE_ACTIVE_VERSION_OFFSET,
            CBM_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
            result->conflict.active_version, true) ||
        !runtime_wire_string_encode(
            out + RENDEZVOUS_RESPONSE_ACTIVE_BUILD_OFFSET,
            CBM_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP,
            result->conflict.active_build_fingerprint, true) ||
        !runtime_wire_string_encode(
            out + RENDEZVOUS_RESPONSE_REQUESTED_VERSION_OFFSET,
            CBM_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
            result->conflict.requested_version, true) ||
        !runtime_wire_string_encode(
            out + RENDEZVOUS_RESPONSE_REQUESTED_BUILD_OFFSET,
            CBM_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP,
            result->conflict.requested_build_fingerprint, true) ||
        !runtime_wire_string_encode(
            out + RENDEZVOUS_RESPONSE_MESSAGE_OFFSET,
...
static bool runtime_hello_response_decode(
    ...
    result->status =
        (cbm_daemon_runtime_connect_status_t)runtime_get_u32(
            payload + RENDEZVOUS_RESPONSE_CONNECT_STATUS_OFFSET);
    result->hello_status =
        (cbm_daemon_hello_status_t)runtime_get_u32(
            payload + RENDEZVOUS_RESPONSE_HELLO_STATUS_OFFSET);
    result->client_id = runtime_get_u64(
        payload + RENDEZVOUS_RESPONSE_CLIENT_ID_OFFSET);
    result->authenticated_process_id =
    ...
    if (!runtime_wire_string_decode(
            payload + RENDEZVOUS_RESPONSE_ACTIVE_VERSION_OFFSET,
            CBM_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
            result->conflict.active_version, true) ||
        !runtime_wire_string_decode(
            ...
            result->conflict.active_build_fingerprint, true) ||
        !runtime_wire_string_decode(
            payload + RENDEZVOUS_RESPONSE_REQUESTED_VERSION_OFFSET,
            CBM_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
            result->conflict.requested_version, true) ||
        !runtime_wire_string_decode(
            payload + RENDEZVOUS_RESPONSE_REQUESTED_BUILD_OFFSET,
            CBM_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP,
            result->conflict.requested_build_fingerprint, true) ||
        !runtime_wire_string_decode(
    ...
    if (result->status == CBM_DAEMON_RUNTIME_CONNECT_CONFLICT) {
        ...
        return result->hello_status >= CBM_DAEMON_HELLO_VERSION_CONFLICT &&
               ...
               result->client_id == CBM_DAEMON_CLIENT_ID_INVALID &&
               result->authenticated_process_id == 0 &&
               cbm_daemon_conflict_format(&result->conflict, expected,
                                          sizeof(expected)) &&
...
static bool runtime_send_hello_response(
    cbm_daemon_ipc_connection_t *connection,
    const cbm_daemon_runtime_connect_result_t *result) {
...
static bool runtime_worker_send_frame(cbm_daemon_runtime_worker_t *worker,
    ...
    bool sent = worker->connection &&
                cbm_daemon_ipc_send_frame(worker->connection, type,
                                          (uint16_t)operation, payload, length);
...
static bool runtime_worker_send_application_response(
    ...
    bool suppress_when_disconnecting) {
    if (!worker ||
    ...
    uint64_t wire_length = APPLICATION_RESPONSE_PREFIX_SIZE +
    ...
    if (response_length > 0) {
        memcpy(wire + APPLICATION_RESPONSE_PREFIX_SIZE, response,
               response_length);
    }

    cbm_mutex_lock(&worker->send_mutex);
    bool disconnected = suppress_when_disconnecting &&
                        atomic_load_explicit(&worker->disconnecting,
                                             memory_order_acquire);
    bool sent = !disconnected && worker->connection &&
                cbm_daemon_ipc_send_frame(
                    worker->connection, CBM_DAEMON_FRAME_RESPONSE,
...
static void runtime_service_begin_stopping_locked(
    cbm_daemon_runtime_service_t *service, uint64_t deadline, bool emergency) {
...
static void runtime_service_interrupt_connections_except(
    cbm_daemon_runtime_service_t *service,
...
static void runtime_service_interrupt_connections(
    cbm_daemon_runtime_service_t *service) {
...
static void runtime_worker_disconnect(cbm_daemon_runtime_worker_t *worker) {
    ...
    if (worker->admission_committed) {
        ...
        if (service->committed_clients == 0) {
            ...
            runtime_service_begin_stopping_locked(
                service, shutdown_deadline, false);
        }
    }
    cbm_mutex_unlock(&service->mutex);
    ...
    if (worker->application_session_opened && !worker->application_cancelled) {
        ...
        service->application.session_cancel(
            service->application.context, worker->application_session);
    }
    if (cbm_daemon_coordinator_state(service->coordinator) ==
        CBM_DAEMON_COORDINATOR_STOPPING) {
...
static bool runtime_worker_commit_admission(
    cbm_daemon_runtime_worker_t *worker) {
    cbm_daemon_runtime_service_t *service = worker->service;
    cbm_mutex_lock(&service->mutex);
    bool committed = service->state == CBM_DAEMON_RUNTIME_SERVICE_RUNNING &&
                     worker->admitted && !worker->admission_committed;
    if (committed) {
...
static bool runtime_worker_handle_subscribe(cbm_daemon_runtime_worker_t *worker,
    ...
    cbm_daemon_subscription_result_t result = cbm_daemon_job_subscribe(
        worker->service->coordinator, worker->client_id, project_key,
        &subscription_id);
    ...
    return runtime_worker_send_frame(worker, CBM_DAEMON_FRAME_RESPONSE,
                                     CBM_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE,
...
static bool runtime_worker_handle_unsubscribe(cbm_daemon_runtime_worker_t *worker,
                                              ...
                                              uint32_t length) {
    if (!runtime_worker_service_running(worker) || !payload ||
        length != UNSUBSCRIBE_REQUEST_SIZE) {
        return false;
    }
    cbm_daemon_subscription_id_t subscription_id = runtime_get_u64(payload);
    bool removed = cbm_daemon_job_unsubscribe(worker->service->coordinator,
                                              worker->client_id, subscription_id);
    return runtime_worker_send_status(
        worker, CBM_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE, removed);
}
...
static void *runtime_application_worker(void *opaque) {
    ...
    cbm_daemon_runtime_application_status_t status =
        service->application.request(
            ...
            worker->application_request, worker->application_request_length,
    ...
    bool sent = runtime_worker_send_application_response(
        worker, worker->application_request_token, status, response,
        response_length, true);
    free(response);
    if (!sent && !atomic_load_explicit(&worker->disconnecting,
                                       memory_order_acquire)) {
        cbm_daemon_ipc_connection_interrupt(worker->connection);
    }
    atomic_store_explicit(&worker->application_thread_done, true,
                          memory_order_release);
    return NULL;
}

static bool runtime_worker_reap_application(
    cbm_daemon_runtime_worker_t *worker, bool wait) {
    if (!worker->application_thread_started) {
        return true;
    }
    if (!wait &&
        !atomic_load_explicit(&worker->application_thread_done,
                              memory_order_acquire)) {
        return false;
    }
    ...
    atomic_store_explicit(&worker->application_thread_done, false,
                          memory_order_release);
    return true;
}

static bool runtime_worker_handle_application(
    cbm_daemon_runtime_worker_t *worker, const uint8_t *payload,
    uint32_t length) {
    if (!payload || length < APPLICATION_REQUEST_PREFIX_SIZE) {
        return false;
    }
    cbm_daemon_runtime_application_token_t request_token =
        runtime_get_u64(payload);
    ...
    if (!service->application.request) {
        return runtime_worker_send_application_response(
            ...
            CBM_DAEMON_RUNTIME_APPLICATION_UNAVAILABLE, NULL, 0, false);
    }
    (void)runtime_worker_reap_application(worker, false);
    if (worker->application_thread_started) {
        return runtime_worker_send_application_response(
            worker, request_token, CBM_DAEMON_RUNTIME_APPLICATION_BUSY, NULL,
            0, false);
    }
    ...
    if (request_length > 0) {
        ...
        if (!request_copy) {
            return runtime_worker_send_application_response(
                worker, request_token,
                CBM_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR, NULL, 0, false);
        }
        memcpy(request_copy, payload + APPLICATION_REQUEST_PREFIX_SIZE,
               request_length);
    }
    ...
    atomic_store_explicit(&worker->application_thread_done, false,
                          memory_order_release);
    if (cbm_thread_create(&worker->application_thread,
...
static bool runtime_worker_handle_application_cancel(
    cbm_daemon_runtime_worker_t *worker, const uint8_t *payload,
    uint32_t length) {
    ...
    if (worker->application_thread_started &&
        ...
        !atomic_load_explicit(&worker->application_thread_done,
                              memory_order_acquire)) {
        worker->service->application.request_cancel(
            worker->service->application.context,
...
static bool runtime_worker_handle_disconnect(cbm_daemon_runtime_worker_t *worker,
                                             uint32_t length) {
    if (length != 0) {
        return false;
    }
    ...
    bool sent = runtime_worker_send_status(
        worker, CBM_DAEMON_RUNTIME_OP_DISCONNECT, true);
    cbm_mutex_lock(&service->mutex);
...
static bool runtime_activation_peer_matches_claim(
    cbm_daemon_runtime_service_t *service, uint64_t process_id,
    const char *claimed_build) {
    if (!service || process_id == 0 || !claimed_build) {
        return false;
    }
    bool active_image = runtime_process_image_reference_matches_process(
        &service->active_image, process_id);
    if (active_image &&
        strcmp(claimed_build, service->identity.build_fingerprint) == 0) {
        return true;
    }
    char peer_fingerprint[CBM_DAEMON_BUILD_FINGERPRINT_SIZE];
    return cbm_daemon_runtime_process_build_fingerprint(
               process_id, peer_fingerprint) &&
           strcmp(peer_fingerprint, claimed_build) == 0;
}

static void runtime_worker_handle_activation_shutdown(
    cbm_daemon_runtime_worker_t *worker, const uint8_t *payload,
    uint32_t length) {
    cbm_daemon_runtime_service_t *service = worker->service;
    ...
    bool request_valid =
        ...
        runtime_activation_request_decode(payload, &action,
                                          requested_version,
                                          requested_build);
    bool peer_verified = request_valid &&
                         runtime_activation_peer_matches_claim(
                             service, worker->peer_process_id,
                             requested_build);
    if (peer_verified) {
        uint64_t deadline =
            runtime_deadline_after(service->shutdown_timeout_ms);
        ...
        result.active_connections =
            service->active_connections > 0
                ? (uint64_t)(service->active_connections - 1U)
                : 0;
        if (service->state == CBM_DAEMON_RUNTIME_SERVICE_RUNNING) {
    ...
    if (result.accepted) {
        ...
        cbm_log_info("daemon.activation_shutdown", "requester_pid",
                     requester_pid, "requester_build", requested_build,
                     "action", runtime_activation_action_text(action),
                     "active_clients", active_clients,
                     "active_connections", active_connections);
    }
    bool response_sent = encoded && runtime_worker_send_frame(
                                        worker, CBM_DAEMON_FRAME_RESPONSE,
                                        CBM_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN,
                                        response,
                                        (uint32_t)sizeof(response));
    if (result.accepted && !response_sent) {
        cbm_log_error("daemon.activation_shutdown_ack_failed", "shutdown",
                      "accepted", "requester_build", requested_build,
                      "action", runtime_activation_action_text(action));
    }
...
static void *runtime_connection_worker(void *opaque) {
    ...
    if (frame.flags == CBM_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN) {
        runtime_worker_handle_activation_shutdown(worker, payload,
                                                  frame.length);
        free(payload);
        return NULL;
    }
    if (frame.flags != CBM_DAEMON_RUNTIME_OP_HELLO ||
        ...
        !runtime_rendezvous_request_decode(payload, requested_version,
                                           requested_build)) {
        free(payload);
        runtime_worker_finish(worker);
        return NULL;
    ...
    cbm_daemon_hello_status_t hello_status = runtime_rendezvous_compare(
        service->identity.semantic_version,
        service->identity.build_fingerprint, requested_version,
    ...
    if (hello_status != CBM_DAEMON_HELLO_COMPATIBLE) {
        ...
        (void)cbm_daemon_conflict_format(&hello_result.conflict,
                                         hello_result.message,
                                         sizeof(hello_result.message));
        if (!cbm_daemon_conflict_log_append(service->conflict_log_path,
                                            &hello_result.conflict,
            ...
            cbm_log_error("daemon.conflict_log_append_failed", "path",
    ...
    bool peer_image_verified =
        runtime_process_image_reference_matches_process(
    ...
    if (!peer_image_verified) {
        ...
        peer_image_fingerprinted =
            cbm_daemon_runtime_process_build_fingerprint(
                worker->peer_process_id, peer_fingerprint);
        peer_image_verified = peer_image_fingerprinted &&
                              strcmp(peer_fingerprint, requested_build) == 0 &&
                              strcmp(peer_fingerprint,
                                     service->identity.build_fingerprint) == 0;
    }
    if (!peer_image_verified) {
        cbm_log_error("daemon.client_image_rejected", "reason",
                      peer_image_fingerprinted ? "fingerprint_mismatch"
                                               : "image_unverifiable");
        runtime_worker_finish(worker);
        return NULL;
    }
    ...
    if (service->application.request) {
        ...
        if (!worker->application_session) {
            runtime_result_rejected(&hello_result,
                                    "CBM daemon session initialization failed");
            cbm_mutex_lock(&service->mutex);
            worker->final_response_inflight = true;
            cbm_mutex_unlock(&service->mutex);
            runtime_worker_disconnect(worker);
            (void)runtime_send_hello_response(worker->connection,
                                              &hello_result);
            cbm_mutex_lock(&service->mutex);
            worker->final_response_inflight = false;
            cbm_mutex_unlock(&service->mutex);
    ...
    while (keep_running && runtime_worker_service_running(worker)) {
        ...
        payload = NULL;
        received = cbm_daemon_ipc_receive_frame(worker->connection,
                                                CBM_DAEMON_IPC_WAIT_FOREVER,
                                                &frame, &payload);
        if (received != 1 || frame.type != CBM_DAEMON_FRAME_REQUEST) {
            free(payload);
            break;
        ...
        switch (frame.flags) {
            case CBM_DAEMON_RUNTIME_OP_HEARTBEAT: {
                bool valid = frame.length == 0 &&
                             cbm_daemon_client_heartbeat(service->coordinator,
                                                         client_id, cbm_now_ms());
                keep_running = valid && runtime_worker_send_status(
                                                   worker,
                                                   CBM_DAEMON_RUNTIME_OP_HEARTBEAT,
                                                   true);
                break;
            }
            case CBM_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE:
                keep_running = runtime_worker_handle_subscribe(
                    worker, payload, frame.length);
                break;
            case CBM_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE:
                keep_running = runtime_worker_handle_unsubscribe(
                    worker, payload, frame.length);
                break;
            ...
            case CBM_DAEMON_RUNTIME_OP_APPLICATION_CANCEL:
                keep_running = runtime_worker_handle_application_cancel(
                    worker, payload, frame.length);
                break;
...
static void runtime_worker_reset_after_join(cbm_daemon_runtime_worker_t *worker) {
    ...
    worker->application_request_token =
        CBM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    worker->last_application_request_token =
        CBM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    worker->application_request = NULL;
    worker->application_request_length = 0;
...
static void runtime_reject_inline(cbm_daemon_ipc_connection_t *connection,
                                  const char *message) {
    cbm_daemon_runtime_connect_result_t result;
...
static void runtime_accept_connection(cbm_daemon_runtime_service_t *service,
    ...
    if (worker) {
        ...
        worker->in_use = true;
        atomic_store_explicit(&worker->done, false, memory_order_release);
        atomic_store_explicit(&worker->disconnecting, false,
                              memory_order_release);
        atomic_store_explicit(&worker->application_thread_done, false,
                              memory_order_release);
        service->active_connections++;
        int created = cbm_thread_create(&worker->thread,
                                        RUNTIME_WORKER_STACK_SIZE,
...
static bool runtime_service_can_exit(cbm_daemon_runtime_service_t *service,
                                     uint64_t now_ms) {
    bool can_exit = false;
    cbm_mutex_lock(&service->mutex);
    if (service->state == CBM_DAEMON_RUNTIME_SERVICE_STOPPING &&
        service->active_connections == 0) {
        ...
        bool deadline_reached = service->stop_deadline_ms != 0 &&
                                now_ms >= service->stop_deadline_ms;
...
static void *runtime_accept_loop(void *opaque) {
    ...
    for (;;) {
        ...
        int accepted = cbm_daemon_ipc_accept(service->listener,
                                             RUNTIME_ACCEPT_POLL_MS,
                                             &connection);
...
static bool runtime_application_callbacks_valid(
    ...
    bool any = application->session_open || application->request ||
               application->request_cancel || application->session_cancel ||
               application->session_close;
    bool all = application->session_open && application->request &&
...
static void runtime_startup_lock_release_complete(
    cbm_daemon_ipc_startup_lock_t **lock_io, uint32_t timeout_ms) {
    uint64_t deadline = runtime_deadline_after(timeout_ms);
...
static void runtime_service_destroy_unstarted(
    cbm_daemon_runtime_service_t *service) {
    if (!service) {
        return;
    }
...
cbm_daemon_runtime_service_t *cbm_daemon_runtime_service_start_reserved(
    ...
    if (!reservation_io || !*reservation_io || !config || !config->endpoint ||
        !config->conflict_log_path ||
        config->conflict_log_cap_bytes == 0 || config->max_clients == 0 ||
        config->max_clients > RUNTIME_MAX_CLIENTS_HARD ||
    ...
    if (!runtime_process_image_reference_acquire(
            runtime_current_process_id(), &active_image,
            active_process_fingerprint) ||
        strcmp(active_process_fingerprint,
               config->identity.build_fingerprint) != 0) {
        cbm_log_error("daemon.runtime.start_failed", "stage",
    ...
    bool copied_identity = runtime_bounded_length(config->identity.semantic_version,
                                                  sizeof(service->semantic_version),
                                                  &version_length) &&
                           runtime_bounded_length(config->identity.build_fingerprint,
                                                  sizeof(service->build_fingerprint),
                                                  &build_length);
    if (copied_identity) {
        memcpy(service->semantic_version, config->identity.semantic_version,
               version_length + 1);
        memcpy(service->build_fingerprint, config->identity.build_fingerprint,
               build_length + 1);
    }
    ...
    service->listener =
        cbm_daemon_ipc_listen_reserved(config->endpoint, reservation_io);
    if (!service->listener) {
        cbm_log_error("daemon.runtime.start_failed", "stage",
                      "listener_handoff");
        runtime_service_destroy_unstarted(service);
    ...
    if (cbm_thread_create(&service->accept_thread, RUNTIME_WORKER_STACK_SIZE,
                          runtime_accept_loop, service) != 0) {
        cbm_log_error("daemon.runtime.start_failed", "stage",
                      "accept_thread");
        service->state = CBM_DAEMON_RUNTIME_SERVICE_EXITED;
        runtime_service_destroy_unstarted(service);
...
cbm_daemon_runtime_service_t *cbm_daemon_runtime_service_start(
    ...
    if (cbm_daemon_ipc_startup_lock_try_acquire(
            config->endpoint, &startup_lock) != 1) {
        return NULL;
    }
    int generation = cbm_daemon_ipc_generation_probe_under_startup_lock(
        config->endpoint, startup_lock);
    if (generation != 0 ||
        !cbm_daemon_ipc_startup_lock_prepare_handoff(startup_lock)) {
        runtime_startup_lock_release_complete(
            &startup_lock, config->shutdown_timeout_ms);
        return NULL;
    }
    ...
    if (cbm_daemon_ipc_lifetime_reservation_try_acquire(
            config->endpoint, &reservation) != 1) {
        if (!runtime_participant_guard_release_complete(
                &participant_guard, config->shutdown_timeout_ms)) {
...
size_t cbm_daemon_runtime_service_active_clients(
    cbm_daemon_runtime_service_t *service) {
    if (!service) {
        return 0;
    }
...
size_t cbm_daemon_runtime_service_job_subscribers(
    cbm_daemon_runtime_service_t *service, const char *project_key) {
...
uint64_t cbm_daemon_runtime_service_client_process_id(
    ...
    for (size_t i = 0; i < service->worker_capacity; i++) {
        cbm_daemon_runtime_worker_t *worker = &service->workers[i];
        if (worker->in_use && worker->admitted &&
            worker->admission_committed && worker->client_id == client_id) {
...
static bool runtime_wait_for_count(cbm_daemon_runtime_service_t *service,
    ...
    uint64_t deadline = runtime_deadline_after(timeout_ms);
    for (;;) {
        size_t actual = connections
                            ? cbm_daemon_runtime_service_active_connections(service)
...
bool cbm_daemon_runtime_service_wait_for_clients(
...
bool cbm_daemon_runtime_service_wait_exited(
    cbm_daemon_runtime_service_t *service, uint32_t timeout_ms) {
    if (!service) {
        return false;
    }
    uint64_t deadline = runtime_deadline_after(timeout_ms);
    for (;;) {
        if (cbm_daemon_runtime_service_state(service) ==
            ...
            atomic_load_explicit(&service->accept_thread_done,
                                 memory_order_acquire)) {
            return true;
        }
        if (cbm_now_ms() >= deadline) {
...
bool cbm_daemon_runtime_service_job_reaped(
    cbm_daemon_runtime_service_t *service, const char *project_key) {
    return service && cbm_daemon_job_reaped(service->coordinator, project_key,
                                             cbm_now_ms());
}
...
bool cbm_daemon_runtime_service_free(cbm_daemon_runtime_service_t *service) {
    if (!service || cbm_daemon_runtime_service_state(service) !=
                        CBM_DAEMON_RUNTIME_SERVICE_EXITED) {
        return false;
    }
    ...
    if (!runtime_participant_guard_release_complete(
            &service->owned_participant_guard,
...
bool cbm_daemon_runtime_request_activation_shutdown(
    ...
    int received = sent ? cbm_daemon_ipc_receive_frame_bounded(
                        ...
                        : 0;
    bool valid =
        received == 1 && frame.type == CBM_DAEMON_FRAME_RESPONSE &&
...
cbm_daemon_runtime_client_t *cbm_daemon_runtime_client_connect(
    ...
    uint8_t *payload = NULL;
    int received = cbm_daemon_ipc_receive_frame_bounded(
        connection, timeout_ms, CBM_DAEMON_RENDEZVOUS_RESPONSE_SIZE,
        &frame, &payload);
...
static bool runtime_client_exchange(cbm_daemon_runtime_client_t *client,
                                    ...
                                    uint32_t timeout_ms, uint8_t **response_out,
    ...
    *response_out = NULL;
    ...
    bool sent = cbm_daemon_ipc_send_frame(
        connection, CBM_DAEMON_FRAME_REQUEST, (uint16_t)operation, request,
        request_length);
    cbm_mutex_unlock(&client->send_mutex);
    cbm_daemon_frame_t frame = {0};
    uint8_t *payload = NULL;
    int received = sent ? cbm_daemon_ipc_receive_frame(
                              connection, timeout_ms, &frame, &payload)
                        : -1;
    bool valid = sent && received == 1 &&
                 frame.type == CBM_DAEMON_FRAME_RESPONSE &&
                 frame.flags == (uint16_t)operation &&
                 frame.length == expected_response_length;
    cbm_mutex_lock(&client->state_mutex);
...
cbm_daemon_subscription_result_t cbm_daemon_runtime_client_job_subscribe(
    ...
    if (!client || !subscription_id_out ||
        !runtime_bounded_length(project_key,
                                CBM_DAEMON_RUNTIME_PROJECT_KEY_MAX + 1,
                                &key_length) ||
        key_length == 0 || key_length > CBM_DAEMON_RUNTIME_PROJECT_KEY_MAX) {
        return CBM_DAEMON_SUBSCRIPTION_REJECTED;
    }
    uint8_t request[SUBSCRIBE_REQUEST_PREFIX_SIZE +
                    CBM_DAEMON_RUNTIME_PROJECT_KEY_MAX];
    runtime_put_u32(request, (uint32_t)key_length);
    memcpy(request + SUBSCRIBE_REQUEST_PREFIX_SIZE, project_key, key_length);
    uint8_t *response = NULL;
    if (!runtime_client_exchange(client, CBM_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE,
                                 request,
                                 (uint32_t)(SUBSCRIBE_REQUEST_PREFIX_SIZE + key_length),
                                 timeout_ms, &response, SUBSCRIBE_RESPONSE_SIZE)) {
        return CBM_DAEMON_SUBSCRIPTION_REJECTED;
    }
    ...
    free(response);
    if ((result != CBM_DAEMON_SUBSCRIPTION_STARTED &&
         result != CBM_DAEMON_SUBSCRIPTION_JOINED) ||
...
bool cbm_daemon_runtime_client_job_unsubscribe(
    ...
    uint8_t *response = NULL;
    if (!runtime_client_exchange(client,
                                 CBM_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE,
                                 request, (uint32_t)sizeof(request), timeout_ms,
...
bool cbm_daemon_runtime_client_heartbeat(cbm_daemon_runtime_client_t *client,
    ...
    uint8_t *response = NULL;
    if (!runtime_client_exchange(client, CBM_DAEMON_RUNTIME_OP_HEARTBEAT,
                                 NULL, 0, timeout_ms, &response,
...
bool cbm_daemon_runtime_client_application_token_reserve(
    ...
    bool valid = client->usable && !client->closing && client->connection &&
                 client->active_application_token ==
                     CBM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID &&
                 client->next_application_token ==
                     client->last_started_application_token &&
...
cbm_daemon_runtime_client_application_cancel(
    ...
    if (client->active_application_token == request_token) {
        ...
        dispatch = client->application_request_sent &&
                   !client->application_cancel_sent;
    } else if (client->active_application_token ==
                   CBM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID &&
    ...
    dispatch = client->usable && !client->closing && client->connection &&
               ...
               client->application_request_sent &&
               !client->application_cancel_sent;
    if (dispatch) {
        client->application_cancel_sent = true;
        connection = client->connection;
    } else {
        unavailable = !client->usable || client->closing ||
                      !client->connection;
        already_sent = client->usable && !client->closing &&
                       client->connection &&
                       client->active_application_token == request_token &&
                       client->application_cancel_sent;
    }
    cbm_mutex_unlock(&client->state_mutex);
    bool sent = !dispatch || cbm_daemon_ipc_send_frame(
                                 connection, CBM_DAEMON_FRAME_REQUEST,
...
cbm_daemon_runtime_client_application_request_tagged(
    ...
    uint64_t wire_length = APPLICATION_REQUEST_PREFIX_SIZE +
    ...
    if (request_length > 0) {
        memcpy(wire + APPLICATION_REQUEST_PREFIX_SIZE, request,
               request_length);
    }
    ...
    send_cancel = sent && !client->closing &&
                  client->pending_cancel_token == request_token &&
                  !client->application_cancel_sent;
    if (send_cancel) {
        client->application_cancel_sent = true;
    ...
    int received = sent ? cbm_daemon_ipc_receive_frame(
                        ...
                        : -1;
    bool protocol_valid =
        sent && received == 1 && frame.type == CBM_DAEMON_FRAME_RESPONSE &&
    ...
    uint32_t response_length = 0;
    if (protocol_valid) {
        cbm_daemon_runtime_application_token_t response_token =
            runtime_get_u64(payload);
        status =
            (cbm_daemon_runtime_application_status_t)runtime_get_u32(payload + 8);
    ...
    if (protocol_valid && response_length > 0) {
        ...
        if (copied) {
            memcpy(response_copy, payload + APPLICATION_RESPONSE_PREFIX_SIZE,
                   response_length);
        }
    }
    free(payload);
...
cbm_daemon_runtime_client_application_request(
    ...
    cbm_daemon_runtime_application_token_t request_token =
        CBM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    if (!cbm_daemon_runtime_client_application_token_reserve(
            client, &request_token)) {
        return CBM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    }
    return cbm_daemon_runtime_client_application_request_tagged(
        client, request_token, request, request_length, response_out,
...
bool cbm_daemon_runtime_client_close_finish(cbm_daemon_runtime_client_t *client,
    ...
    if (can_disconnect) {
        cbm_mutex_lock(&client->send_mutex);
        disconnect_sent = cbm_daemon_ipc_send_frame(
            connection, CBM_DAEMON_FRAME_REQUEST,
    ...
    if (disconnect_sent) {
        ...
        int received = cbm_daemon_ipc_receive_frame(
            connection, timeout_ms, &frame, &response);
        acknowledged =
            received == 1 && frame.type == CBM_DAEMON_FRAME_RESPONSE &&
